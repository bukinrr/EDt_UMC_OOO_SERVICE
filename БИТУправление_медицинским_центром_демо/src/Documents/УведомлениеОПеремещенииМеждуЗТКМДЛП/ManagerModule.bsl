...
Функция ПоследовательностьОпераций(ДокументСсылка) Экспорт
	...
	Операция = ИнтеграцияМДЛП.ДобавитьОперациюВПоследовательность(Таблица, 0, Исходящее, Перечисления.ОперацииОбменаМДЛП.ПередачаДанных_ПеремещениеЛПМеждуЗТК);
...
Функция ЗапросСтатусаОформления(ДокументОснование) Экспорт
	...
	Запрос = ИнтеграцияМДЛППереопределяемый.ЗапросСтатусаОформленияУведомленияОПеремещенииМеждуЗТК(ДокументОснование);
...
Процедура ПриЗаполненииДокументовПанелиМаркировкиМДЛП(ТаблицаДокументы) Экспорт
	...
	Описание = ИнтеграцияМДЛП.ДобавитьДокументНаПанельМаркировки(
		ТаблицаДокументы,
		Метаданные.Документы.УведомлениеОПеремещенииМеждуЗТКМДЛП,
		НСтр("ru = 'Перемещение между зонами таможенного контроля'"),
...
Функция ПанельМаркировкиМДЛПТекстЗапросаОформите() Экспорт
	...
	Возврат ИнтеграцияМДЛППереопределяемый.УведомлениеОПеремещенииМеждуЗТКТекстЗапросаОформите();
...
Функция ПанельМаркировкиМДЛПТекстЗапросаОтработайте() Экспорт
	...
	|		Документ.УведомлениеОПеремещенииМеждуЗТКМДЛП КАК Уведомления
	|	ПО
...
Функция ПанельМаркировкиМДЛПТекстЗапросаДинамическогоСпискаКОформлению() Экспорт
	...
	ТекстЗапроса = ИнтеграцияМДЛППереопределяемый.УведомлениеОПеремещенииМеждуЗТКТекстЗапросаДинамическогоСпискаКОформлению();
...
Функция СообщениеКПередаче(ДокументСсылка, ДальнейшееДействие, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Возврат УведомлениеОПеремещенииМеждуЗТК(ДокументСсылка);
...
Функция ПредставлениеДокумента() Экспорт
	...
	ВозвращаемоеЗначение.КомандаСоздать                       = НСтр("ru = 'Создать уведомление о перемещении между зонами таможенного контроля МДЛП'");
	ВозвращаемоеЗначение.ИмяКомандыСоздать                    = "СоздатьУведомлениеОПеремещенииМеждуЗТКМДЛП";
	ВозвращаемоеЗначение.ИмяКомандыОткрыть                    = "ОткрытьУведомлениеОПеремещенииМеждуЗТКМДЛП";
	ВозвращаемоеЗначение.ДокументОтсутствуетНетПравНаСоздание = НСтр("ru = 'Уведомление о перемещении между зонами таможенного контроля МДЛП не создано'");
	ВозвращаемоеЗначение.Представление                        = НСтр("ru = 'Уведомление о перемещении между зонами таможенного контроля МДЛП: %1'");
	ВозвращаемоеЗначение.НесколькоДокументовПредставление     = НСтр("ru = 'Уведомление о перемещении между зонами таможенного контроля МДЛП (%1)'");
...
Функция УведомлениеОПеремещенииМеждуЗТК(ДокументСсылка)
	...
	СообщениеКПередаче.Операция = Перечисления.ОперацииОбменаМДЛП.ПередачаДанных_ПеремещениеЛПМеждуЗТК;
	...
	|	Шапка.ЗонаТаможенногоКонтроляОтгрузки.РегистрационныйНомерУчастника  КАК ИдентификаторЗТКОтгрузки,
	|	Шапка.ЗонаТаможенногоКонтроляПриемки.РегистрационныйНомерУчастника   КАК ИдентификаторЗТКПриемки
	|ИЗ
	|	Документ.УведомлениеОПеремещенииМеждуЗТКМДЛП КАК Шапка
	|ГДЕ
	|	Шапка.Ссылка = &Ссылка
	...
	|ИЗ
	|	Документ.УведомлениеОПеремещенииМеждуЗТКМДЛП.Товары КАК Товары
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|		Документ.УведомлениеОПеремещенииМеждуЗТКМДЛП.НомераУпаковок КАК НомераУпаковок
	|	ПО
	...
	|ИЗ
	|	Документ.УведомлениеОПеремещенииМеждуЗТКМДЛП.ТранспортныеУпаковки КАК НомераУпаковок
	|ГДЕ
	...
	ИнтеграцияМДЛП.ЗаполнитьСвойствоXDTO(Уведомление, "custom_shipper_id", Шапка.ИдентификаторЗТКОтгрузки, СообщениеКПередаче);
	ИнтеграцияМДЛП.ЗаполнитьСвойствоXDTO(Уведомление, "custom_receiver_id", Шапка.ИдентификаторЗТКПриемки, СообщениеКПередаче);
