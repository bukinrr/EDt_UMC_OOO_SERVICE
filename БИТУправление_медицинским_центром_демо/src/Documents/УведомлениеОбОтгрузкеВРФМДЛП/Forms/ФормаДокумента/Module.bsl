...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ПараметрыЗаписи.РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ОценкаПроизводительности") Тогда
			...
			МодульОценкаПроизводительностиКлиент.НачатьЗамерВремени(Истина, "Документ.УведомлениеОбОтгрузкеВРФМДЛП.Форма.ФормаДокумента.Провести");
		КонецЕсли;
	КонецЕсли;
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	Оповестить("Запись_УведомлениеОбОтгрузкеВРФМДЛП", ПараметрыЗаписи, Объект.Ссылка);
...
Процедура ПриСозданииЧтенииНаСервере()
	...
	Если Объект.СхемаАкцептования = Перечисления.СхемыАкцептованияМДЛП.ОбратныйПорядок Тогда
		...
		Элементы.ИдентификаторЗТК.Видимость = Истина;
		Элементы.ИдентификаторПродавца.Видимость = Истина;
	Иначе
...
Процедура УстановитьВидимостьЭлементовПоОперации()
	...
	Документы.УведомлениеОбОтгрузкеВРФМДЛП.ЗаполнитьИменаРеквизитовПоТипуОперации(Объект.Операция, ВсеРеквизиты, РеквизитыОперации);
...
Процедура ОбновитьСтатусУведомления()
	...
	Если Объект.СхемаАкцептования = Перечисления.СхемыАкцептованияМДЛП.ОбратныйПорядок Тогда
		Если ТекущийСтатус.Статус <> Перечисления.СтатусыИнформированияМДЛП.ПринятоИзМДЛП Тогда
			...
			ИменаЭлементов.Добавить(Элементы.Импортер.Имя);
