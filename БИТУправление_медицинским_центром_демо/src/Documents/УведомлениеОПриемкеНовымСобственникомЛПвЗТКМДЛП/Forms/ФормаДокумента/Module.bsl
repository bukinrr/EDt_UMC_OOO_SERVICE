...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ПараметрыЗаписи.РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ОценкаПроизводительности") Тогда
			...
			МодульОценкаПроизводительностиКлиент.НачатьЗамерВремени(Истина, "Документ.УведомлениеОПриемкеНовымСобственникомЛПвЗТКМДЛП.Форма.ФормаДокумента.Провести");
		КонецЕсли;
	КонецЕсли;
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	Оповестить("Запись_УведомлениеОПриемкеНовымСобственникомЛПвЗТКМДЛП", ПараметрыЗаписи, Объект.Ссылка);
...
Процедура НомераУпаковокВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле = Элементы.НомераУпаковокЗонаТаможенногоКонтроля Тогда
		СтандартнаяОбработка = Ложь;
		ОбработатьВводДанныхЗТК(ВыбраннаяСтрока, Элемент.Имя);
	КонецЕсли;
...
Процедура ТранспортныеУпаковкиВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле = Элементы.ТранспортныеУпаковкиЗонаТаможенногоКонтроля Тогда
		СтандартнаяОбработка = Ложь;
		ОбработатьВводДанныхЗТК(ВыбраннаяСтрока, Элемент.Имя);
	КонецЕсли;
...
#Область НомераУпаковок
	...
	//
	Элемент = УсловноеОформление.Элементы.Добавить();
	Элемент.Поля.Элементы.Добавить().Поле = Новый ПолеКомпоновкиДанных(Элементы.НомераУпаковокЗонаТаможенногоКонтроля.Имя);
	...
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(Элемент.Отбор,
		"Объект.НомераУпаковок.СостояниеПолученияИнформации", Перечисления.СостоянияПодтвержденияМДЛП.ПустаяСсылка(), ВидСравненияКомпоновкиДанных.НеРавно);
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(Элемент.Отбор,
		"Объект.НомераУпаковок.ЗонаТаможенногоКонтроля",, ВидСравненияКомпоновкиДанных.НеЗаполнено);
	
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", Новый ПолеКомпоновкиДанных("Объект.НомераУпаковок.ИдентификаторЗТК"));
...
#Область ТранспортныеУпаковки
	...
	//
	Элемент = УсловноеОформление.Элементы.Добавить();
	Элемент.Поля.Элементы.Добавить().Поле = Новый ПолеКомпоновкиДанных(Элементы.ТранспортныеУпаковкиЗонаТаможенногоКонтроля.Имя);
	...
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(Элемент.Отбор,
		"Объект.ТранспортныеУпаковки.СостояниеПолученияИнформации", Перечисления.СостоянияПодтвержденияМДЛП.ПустаяСсылка(), ВидСравненияКомпоновкиДанных.НеРавно);
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(Элемент.Отбор,
		"Объект.ТранспортныеУпаковки.ЗонаТаможенногоКонтроля",, ВидСравненияКомпоновкиДанных.НеЗаполнено);
	
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", Новый ПолеКомпоновкиДанных("Объект.ТранспортныеУпаковки.ИдентификаторЗТК"));
...
Процедура ЗаполнитьКонтрагентаПоИдентификаторуНаСервере()
	...
	ИнтеграцияМДЛП.ПолучитьОрганизациюМестоДеятельностиПоИдентификатору(Объект.ИдентификаторКонтрагента, Объект.Контрагент);
...
Процедура ОткрытьФормуСканированияИПроверкиУпаковок_ПослеЗаписи(ЗаписаноУспешно, ДополнительныеПараметры) Экспорт
	...
	ПараметрыСканированияИПроверки.Вставить("ДобавлятьОтсутствующиеВДокументе"        , Ложь);
...
Процедура ОбработатьОкончаниеПроверкиИПодбора(Знач АдресВременногоХранилища)
	...
	Попытка
		...
		ДальнейшиеДействия = Документы.УведомлениеОПриемкеНовымСобственникомЛПвЗТКМДЛП.ПолучитьДальнейшиеДействияПоДокументу(Объект.Ссылка);
...
Процедура ЗаполнитьДанныеКИЗ(Данные)
	...
	Для Каждого КлючЗначение Из Данные Цикл
		...
		Если ЕстьИнформация Тогда
			...
			СтрокаУпаковки.ИдентификаторЗТК = ДанныеУпаковки["customs_point_id"];
			
			Если ЗначениеЗаполнено(СтрокаУпаковки.ИдентификаторЗТК) Тогда
				ИнтеграцияМДЛП.ПолучитьЗонуТаможенногоКонтроляПоИдентификатору(СтрокаУпаковки.ИдентификаторЗТК, СтрокаУпаковки.ЗонаТаможенногоКонтроля);
			КонецЕсли;
			...
			Пока СтекУпаковок.Количество() > 0 Цикл
				...
				Для Каждого ЭлементСостава Из Состав Цикл
					...
					Если ЭтоКИЗ Тогда
						...
						НоваяСтрока.ИдентификаторЗТК = ЭлементСостава["customs_point_id"];
						Если ЗначениеЗаполнено(НоваяСтрока.ИдентификаторЗТК) Тогда
							ИнтеграцияМДЛП.ПолучитьЗонуТаможенногоКонтроляПоИдентификатору(НоваяСтрока.ИдентификаторЗТК, НоваяСтрока.ЗонаТаможенногоКонтроля);
						КонецЕсли;
						
						СтрокаУпаковки.ИдентификаторЗТК = ЭлементСостава["customs_point_id"];
						Если ЗначениеЗаполнено(СтрокаУпаковки.ИдентификаторЗТК) Тогда
							ИнтеграцияМДЛП.ПолучитьЗонуТаможенногоКонтроляПоИдентификатору(СтрокаУпаковки.ИдентификаторЗТК, СтрокаУпаковки.ЗонаТаможенногоКонтроля);
						КонецЕсли;
...
&НаКлиенте
Процедура ОбработатьВводДанныхЗТК(ТекущаяСтрока, ИмяТаблицы)
	
	Если ТекущаяСтрока = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	СтрокаТаблицы = Объект[ИмяТаблицы].НайтиПоИдентификатору(ТекущаяСтрока);
	Если Не ЗначениеЗаполнено(СтрокаТаблицы.ЗонаТаможенногоКонтроля) И ЗначениеЗаполнено(СтрокаТаблицы.ИдентификаторЗТК) Тогда
		Обработчик = Новый ОписаниеОповещения("ОбработатьВводДанныхЗТК_ПослеПолученияДанных", ЭтотОбъект, СтрокаТаблицы.ИдентификаторЗТК);
		ОткрытьФорму("Справочник.ЗоныТаможенногоКонтроляМДЛП.Форма.ЗапросДанныхЗТК", Новый Структура("Идентификатор", СтрокаТаблицы.ИдентификаторЗТК), ЭтотОбъект,,,, Обработчик);
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ОбработатьВводДанныхЗТК_ПослеПолученияДанных(Результат, ТекущаяСтрока) Экспорт
	
	ОбработатьВводДанныхЗТКНаСервере(ТекущаяСтрока);
	
КонецПроцедуры

&НаСервере
Процедура ОбработатьВводДанныхЗТКНаСервере(ИдентификаторЗТК)
	
	ЗТК = Неопределено;
	ИнтеграцияМДЛП.ПолучитьЗонуТаможенногоКонтроляПоИдентификатору(ИдентификаторЗТК, ЗТК);
	
	Если Не ЗначениеЗаполнено(ЗТК) Тогда
		Возврат;
	КонецЕсли;
	
	Для Каждого СтрокаТаблицы Из Объект.НомераУпаковок Цикл
		Если СтрокаТаблицы.ИдентификаторЗТК = ИдентификаторЗТК Тогда
			СтрокаТаблицы.ЗонаТаможенногоКонтроля = ЗТК;
		КонецЕсли;
	КонецЦикла;
	
	Для Каждого СтрокаТаблицы Из Объект.ТранспортныеУпаковки Цикл
		Если СтрокаТаблицы.ИдентификаторЗТК = ИдентификаторЗТК Тогда
			СтрокаТаблицы.ЗонаТаможенногоКонтроля = ЗТК;
		КонецЕсли;
	КонецЦикла;
