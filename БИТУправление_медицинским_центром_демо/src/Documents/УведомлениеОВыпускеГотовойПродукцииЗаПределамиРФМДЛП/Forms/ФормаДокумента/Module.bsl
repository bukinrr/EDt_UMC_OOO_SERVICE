...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ПараметрыЗаписи.РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ОценкаПроизводительности") Тогда
			...
			МодульОценкаПроизводительностиКлиент.НачатьЗамерВремени(Истина, "Документ.УведомлениеОВыпускеГотовойПродукцииЗаПределамиРФМДЛП.Форма.ФормаДокумента.Провести");
		КонецЕсли;
	КонецЕсли;
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	Оповестить("Запись_УведомлениеОВыпускеГотовойПродукцииЗаПределамиРФМДЛП", ПараметрыЗаписи, Объект.Ссылка);
...
Процедура ПриСозданииЧтенииНаСервере()
	...
	Элементы.ЗонаТаможенногоКонтроля.Видимость = Объект.ПередачаСведенийЧерезСКЗКМ;
...
Процедура НастроитьФормуПоОперации()
	...
	Если ЭтоВходящееУведомление Тогда
		...
		Элементы.Упаковщик.ТолькоПросмотр = Ложь;
		Элементы.КонтролирующийКачество.ТолькоПросмотр = Ложь;
