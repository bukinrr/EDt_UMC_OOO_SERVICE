...
Функция ПрочитатьJSONРезультатОперации(СтрокаJSON, СписокОшибок = Неопределено, ОснованиеФискальнойОперации = Неопределено) Экспорт 
	...
	Если Не ПустаяСтрока(СтрокаJSON) Тогда
		...
		//+бит если возникает ошибка типа 502 вместо JSON приходит HTML, что приводит
		// к необрабатываемой ошибке 
		// Было:
		...
		Попытка
			РезультатОперации = ПрочитатьJSON(ЧтениеJSON);
		Исключение
			ВызватьИсключение "Произошла ошибка при обработке запроса, повторите запрос";
		КонецПопытки;
		//-бит
...
Процедура ЗаписатьИсториюПлатежнойОперации(ТипПлатежнойОперации, ДанныеОперации, РезультатВыполнения, ТекстОтветаJSON)
	...
	//+бит На одном из этапов преобразуется в строку и получаем пустую строку
	// может быть и было бы достаточно проверить на заполнение но на всякий случай добавил проверку на тип
	// было:
	//Если ДанныеОперации.ИдентификаторКорзины = Неопределено Тогда 
	// стало:
	Если ДанныеОперации.ИдентификаторКорзины = Неопределено 
		Или (Не ЗначениеЗаполнено(ДанныеОперации.ИдентификаторКорзины)
			И ТипЗнч(РезультатВыполнения) = Тип("Структура")
			И ДанныеОперации.Свойство("ИдентификаторКорзины"))
	Тогда
	//-бит
