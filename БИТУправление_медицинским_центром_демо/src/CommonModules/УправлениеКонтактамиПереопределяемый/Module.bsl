...
Процедура ЗаполнитьКонтактноеЛицоЗадачи(Задача) Экспорт
	
	Если ТипЗнч(Задача.Объект) = Тип("СправочникСсылка.Клиенты")
		И Метаданные.Справочники.Клиенты.Реквизиты.Найти("ЗаконныйПредставитель") <> Неопределено
		И Не ЗначениеЗаполнено(Задача.КонтактноеЛицо)
	Тогда
		
		Если ЗначениеЗаполнено(Задача.Объект.ЗаконныйПредставитель) И 
			ТипЗнч(Задача.Объект.ЗаконныйПредставитель) = Тип("СправочникСсылка.Клиенты")
		Тогда
			Задача.КонтактноеЛицо = Задача.Объект.ЗаконныйПредставитель;
		КонецЕсли;
	ИначеЕсли ТипЗнч(Задача.Объект) = Тип("СправочникСсылка.Сертификаты") Тогда
		Задача.КонтактноеЛицо = Задача.Объект.ВладелецКарты;
	КонецЕсли;
	
КонецПроцедуры

Процедура СформироватьПереопределяемыеЗадачи() Экспорт
