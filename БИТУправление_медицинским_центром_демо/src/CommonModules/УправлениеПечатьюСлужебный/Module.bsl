...
//
Функция МакетИзДвоичныхДанных(ДвоичныеДанныеМакета, СтарыйФормат = Неопределено) Экспорт
	...
	//+бит Необходимо для поддержки старого формата с параметром вида: <...> и для заполнения области
	Если СтарыйФормат <> Неопределено Тогда
		...
		Для Каждого Имя Из мИменXmlФайлов Цикл
			...
			Если мXMLФайл.Количество() > 0 Тогда
				...
				Если Имя = "document.xml" Тогда
					
					НачалоОбласти = "<w:p><w:r><w:t>{v8 Область.Документ}</w:t></w:r></w:p>";
					КонецОбласти = "<w:p><w:r><w:t>{/v8 Область.Документ}</w:t></w:r></w:p>";
					
					XMLСтрока = СтрЗаменить(XMLСтрока, "<w:body>", "<w:body>" + НачалоОбласти);
					
					// Может встречаться несколько "<w:sectPr>" блока
					Номер = СтрНайти(XMLСтрока, "<w:sectPr", НаправлениеПоиска.СКонца);
					
					Если Номер = 0 Тогда
						ВызватьИсключение "Неверный формат .docx файла";
					КонецЕсли;
					
					XMLСтрока = Лев(XMLСтрока, Номер - 1) + КонецОбласти + Прав(XMLСтрока, СтрДлина(XMLСтрока) - Номер + 1); 
...
Процедура РазобратьКонтейнерДокументDOCX(Знач ПолноеИмяФайла, Знач ПутьКСтруктуреФайла) Экспорт
...
Процедура СобратьКонтейнерДокументаDOCX(Знач ПолноеИмяФайла, Знач ПутьКСтруктуреФайла) Экспорт
...
#Область БитПоддержкаРаботыСоСтарымФорматом

Процедура ПодставновкаТаблицДляСтарогоФормата(XMLСтрока, СтруктураТаблиц)
	
			//Файл = Новый Файл(мXMLФайл[0]);
			ЧтениеXML = Новый ЧтениеXML;
					...
					Для Каждого Узел Из мНайденныхУзлов Цикл
						...
						Если УзлыСтрокТаблицы.Количество() > 0 Тогда
							...
							Если РабочийУзел <> Неопределено Тогда
								...
								Если РабочийУзел.ДочерниеУзлы.Количество() = ТабДок.ШиринаТаблицы Тогда
									ТабДок = Таблица.Значение;
									...
									Для НомерСтроки = 1 По ТабДок.ВысотаТаблицы Цикл
										Если ПервыйПроходНеПройден Тогда	
											Для Колонка = 1 По ТабДок.ШиринаТаблицы Цикл
												Если ПервыйПроходНеПройден Тогда
													//             	Если ПервыйПроходНеПройден Тогда
													//				СписокУзловСТекстом = РабочийУзел.ПолучитьЭлементыПоИмени("w:r");
													//				Если СписокУзловСТекстом.Количество() > 0 Тогда
													//					Для Каждого Эл Из СписокУзловСТекстом Цикл
													//						
													//						Если СтрНайти(Эл.ТекстовоеСодержимое, Таблица.Ключ) Тогда
													//							УзелЭталонДляЗаписи = Эл;
													//						Иначе
													//							ДокументDOM.УдалитьДочерний(Эл);
													//						КонецЕсли;
													//						
													//						
													//					КонецЦикла;
													//					
													//				КонецЕсли
													ЭлементыСТекстом = РабочийУзел.ДочерниеУзлы[Колонка-1].ПолучитьЭлементыПоИмени("w:t");
												...
												Иначе
													ТекущийУзел = РабочийУзел.ДочерниеУзлы[Колонка-1];
													РабочийУзел.ВставитьПеред(РабочийУзел.ДочерниеУзлы[Колонка-2].КлонироватьУзел(Истина),РабочийУзел.ДочерниеУзлы[Колонка-1]);
													РабочийУзел.УдалитьДочерний(ТекущийУзел); 
													ЭлементСТекстом = РабочийУзел.ДочерниеУзлы[Колонка-1].ПолучитьЭлементыПоИмени("w:t");
													ТекстЯчейки = ТабДок.Область(НомерСтроки, Колонка).Текст;
			...
			//мТелоДокумента
			//
			//ДокументDOM.("{v8 ТЕСТИНГГГ}")
			
			Для Каждого Эл Из СтруктураТаблиц Цикл
		//Рез = СтрНайти(XMLСтрока, Эл.Ключ);
		//НачалоТаблицы = СтрНайти(XMLСтрока, "<w:tbl>", НаправлениеПоиска.СКонца, Рез);
		//КонецТаблицы = СтрНайти(XMLСтрока, "</w:tbl>", НаправлениеПоиска.СНачала, Рез);
		//Если  Эл.Значение.Колонки.Количество() = ПодсчетКоличестваКолонок(XMLСтрока, НачалоТаблицы, КонецТаблицы) Тогда
		//	ЭталонXMLСтрокиТаблицы = Прав(XMLСтрока, СтрНайти(XMLСтрока, "<w:tr" ,НаправлениеПоиска.СКонца,Рез);
		//	ЭталонXMLСтрокиТаблицы = Лев(XMLСтрокиТаблицы, СтрНайти(XMLСтрокиТаблицы, "</w:tr>", НаправлениеПоиска.СНачала) - 1);   
		//	ЭталонXMLКолонки = Прав(ЭталонXMLСтрокиТаблицы, СтрНайти(ЭталонXMLСтрокиТаблицы, "<w:tc>");
		//	ЭталонXMLКолонки = Лев(ЭталонXMLСтрокиТаблицы, Стр
		//	//НачалоXML = Лев(XMLСтрока, С
		//	//XMLНовойСтроки = ЭталонXMLСтрокиТаблицы;
		//	Для Каждого Строка Из Эл.Значение Цикл
		//	
		//		Для Каждого Колонка из Эл.Значение.Колонки Цикл
		//  			XMLНовойСтроки
		//		КонецЦикла	
		//		
		//	КонецЦикла;
