...
//
Процедура ПроверитьПодписи(Форма, СсылкаНаДвоичныеДанные, ВыделенныеСтроки = Неопределено, ОбработчикЗавершения = Неопределено) Экспорт
	...
	КонецЕсли;
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "ИспользуетсяРЭМД") Тогда
		ДополнительныеПараметры.Вставить("ДвоичныеДанныеЭМД", ИнтеграцияЕГИСЗ_РЭМД.ПолучитьДДСформрованныхЭМД(Форма.Объект.Ссылка, Форма.УникальныйИдентификатор));
	КонецЕсли;
	Если ИмеютсяПодписиФайлов(Форма.Объект.Ссылка) Тогда
		ДополнительныеПараметры.Вставить("ДвоичныеДанныеФайлов", ПолучитьДвоичныеДанныеФайлаЭМД(Форма.Объект.Ссылка));
	КонецЕсли;
	// -БИТ
	
	// +БИТ
	//Если Не Форма.Объект.Зашифрован Тогда
	Если Не ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма.Объект, "Зашифрован")
		Или Не Форма.Объект.Зашифрован Тогда
	// -БИТ
...
// +БИТ
Процедура ПроверитьПодписиПриЗакрытойФормеДокумента(Форма, СсылкаНаДвоичныеДанные, ВыделенныеСтроки = Неопределено, ОбработчикЗавершения = Неопределено) Экспорт
	...
	Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, "ПодписаниеДляРЭМД") Тогда
		ДополнительныеПараметры.Вставить("ДвоичныеДанныеЭМД", ИнтеграцияЕГИСЗ_РЭМД.ПолучитьДДСформрованныхЭМД(Форма.МедицинскийДокумент, Форма.УникальныйИдентификатор));
	КонецЕсли;
	
	Если ИмеютсяПодписиФайлов(Форма.МедицинскийДокумент) Тогда
		ДополнительныеПараметры.Вставить("ДвоичныеДанныеФайлов", ПолучитьДвоичныеДанныеФайлаЭМД(Форма.МедицинскийДокумент));
	КонецЕсли;
...
Функция ПолучитьДвоичныеДанныеФайлаЭМД(Ссылка)
	
	ДанныеФайлов = бит_РаботаСФайлами.ПолучитьДанныеФайловЭМД(Ссылка);
...
Функция ИмеютсяПодписиФайлов(МедицинскийДокумент)
	
	ИмяТипаМД = ДопСерверныеФункции.ИмяСсылочногоТипа(МедицинскийДокумент);
	
	Если ИмяТипаМД = "Справочник.МедицинскиеКарты"
		Или ИмяТипаМД = "Документ.Форма066" // Версия КОРП
		Или ИмяТипаМД = "Документ.СписокНаПрохождениеМедосмотра"
	Тогда
		Возврат Истина;
	ИначеЕсли ИмяТипаМД = "Документ.ПрохождениеМедосмотра"
		Или ИмяТипаМД = "Документ.МедицинскийОтчет"
		Или ИмяТипаМД = "Документ.Прием"
		Или ИмяТипаМД = "Документ.ВрачебнаяКомиссия" // Версия КОРП
	Тогда
		Возврат бит_РаботаСФайлами.ФайлЭМДСохранен(МедицинскийДокумент);
	КонецЕсли;
...
Процедура ПроверитьПодписиПослеПроверкиСтроки(Результат, ДополнительныеПараметры) Экспорт
	...
	// +БИТ
	ДанныеПроверкиПодписи = Новый Структура("Объект, УстановившийПодпись, ДатаПодписи, УникальныйИдентификатор, ДатаПроверкиПодписи, ПодписьВерна, ОписаниеОшибки");
	ЗаполнитьЗначенияСвойств(ДанныеПроверкиПодписи, СтрокаПодписи);
	бит_СертификатыЭЦП.ОбновитьСтатусПроверкиПодписи(ДанныеПроверкиПодписи);
	// -БИТ
...
#Иначе
	...
	Если ПараметрыСканирования = Неопределено Тогда
		...
		ПараметрыСканирования = РаботаСФайламиСлужебныйВызовСервера.ПараметрыСканированияПоТекущемуРабочемуМесту(Истина);
		//-бит
	КонецЕсли;
...
Процедура ПроверитьПодписиЦиклНачало(ДополнительныеПараметры)
	...
	Если ДополнительныеПараметры.Коллекция.Количество() <= ДополнительныеПараметры.Индекс + 1 Тогда
		...
		Если ДополнительныеПараметры.Свойство("ОбработчикЗавершения") Тогда
			ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОбработчикЗавершения, ДополнительныеПараметры);
		КонецЕсли;
		// -БИТ
		Возврат;
	КонецЕсли;
	...
	// +БИТ
	Если ДополнительныеПараметры.Свойство("ДвоичныеДанныеЭМД")
		И ДополнительныеПараметры.СтрокаПодписи.ПодписанФайлЭМД
	Тогда
		ПодписанныеДанные = ДополнительныеПараметры.ДвоичныеДанныеЭМД.Получить(ДополнительныеПараметры.СтрокаПодписи.УникальныйИдентификатор);
	ИначеЕсли ДополнительныеПараметры.Свойство("ДвоичныеДанныеФайлов") Тогда
		УИД_ЭМД = ДополнительныеПараметры.СтрокаПодписи.УИД_ЭМД;
		ДанныеФайлаЭМД = ДополнительныеПараметры.ДвоичныеДанныеФайлов.Получить(УИД_ЭМД);
		ПодписанныеДанные = ДанныеФайлаЭМД.ДвоичныеДанные;
	Иначе
		ПодписанныеДанные = ДополнительныеПараметры.Данные;
	КонецЕсли;
	// -БИТ
...
Процедура ПроверитьПодписиПослеПодготовкиДанных(Данные, ДополнительныеПараметры)
	...
	Если ИспользоватьЭлектроннуюПодписьВМоделиСервиса
	 Или Не МодульЭлектроннаяПодписьКлиент.ПроверятьЭлектронныеПодписиНаСервере() Тогда
		...
		// БИТ.Начало.Изменили, чтобы всегда использовался алгоритм модели сервиса с получением алгоритма подписи перед получением менеджера криптографии.
	...
	Для каждого Элемент Из Коллекция Цикл
		...
		// +БИТ
		ДанныеСтроки.Вставить("ТекстОшибкиПроверкиПодписи",	СтрокаПодписи.ТекстОшибкиПроверкиПодписи);
		...
		Если СтрокаПодписи.ПодписанФайлЭМД Тогда
			Если ДополнительныеПараметры.Свойство("ДвоичныеДанныеЭМД")
				И ТипЗнч(ДополнительныеПараметры.ДвоичныеДанныеЭМД) = Тип("Соответствие")
			Тогда
				ДвоичныеДанныеЭМД = ДополнительныеПараметры.ДвоичныеДанныеЭМД.Получить(СтрокаПодписи.УникальныйИдентификатор);
				Если ЗначениеЗаполнено(ДвоичныеДанныеЭМД) Тогда
					ДанныеСтроки.Вставить("ДвоичныеДанныеЭМД", ДвоичныеДанныеЭМД);
				КонецЕсли;
			КонецЕсли;
		Иначе
			Если ДополнительныеПараметры.Свойство("ДвоичныеДанныеФайлов") Тогда
				ДанныеФайлаЭМД = ДополнительныеПараметры.ДвоичныеДанныеФайлов.Получить(Элемент.УИД_ЭМД);
				ДанныеСтроки.Вставить("ДвоичныеДанныеФайлов", ДанныеФайлаЭМД.ДвоичныеДанные);
			КонецЕсли;
		КонецЕсли;
		// -БИТ
