...
Процедура ОпределитьРазделыСВариантамиОтчетов(Разделы) Экспорт
	...
	Для каждого Подсистема Из Метаданные.Подсистемы Цикл
		Разделы.Добавить(Подсистема,НСтр("ru = 'Отчеты.'" + Подсистема.ПолноеИмя()) )
	КонецЦикла;	                                   
...
Процедура ПриДобавленииДополнительныхОтчетовДоступныхТекущемуПользователю(ДоступныеОтчеты) Экспорт
	...
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ВидОтчет", Перечисления.ВидыДополнительныхВнешнихОбработок.Отчет);
	Запрос.Текст =
	...
	|ГДЕ
	|	ВнешниеОбработки.ВидОбработки = &ВидОтчет"
	;
