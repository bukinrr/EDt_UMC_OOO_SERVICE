...
Функция ПараметрыФискализацииЧека() Экспорт
	...
	//+бит
	ПараметрыФискализации.Вставить("бит_Сторно", Ложь);
	ПараметрыФискализации.Вставить("НеСторнировать", Ложь);
	ПараметрыФискализации.Вставить("бит_ПодключаемоеОборудование", );
	//-бит
...
Функция ПодготовитьДанныеОперации(ПараметрыПодключения, Команда, ПараметрыОперации) Экспорт
	...
	Если Команда = "CheckFiscalization" Тогда
		...
		Если ПараметрыПодключения.ТипОборудования = "ККТ" Тогда
			...
			//+бит
			Если ПараметрыОперации.Свойство("ПодключаемоеОборудование") Тогда
				ПараметрыФискализации.бит_ПодключаемоеОборудование = ПараметрыОперации.ПодключаемоеОборудование;
			КонецЕсли;
			//-бит
	...
	ИначеЕсли Команда = "PrintReceiptCorrection" Тогда
		...
		Иначе
			...
			//+бит Получаем все фискальные операции для которых нужно создать чек сторнирования
			//т.к. в ОУ может быть пробито два чека, а типовая предусматривает только один чек
			//Было:
			...
			//Стало:
			КорректируемыеДокументыОпераций = МенеджерОборудованияВызовСервераПереопределяемый.ДанныеФискальнойОперации(ПараметрыОперации.КорректируемыйДокумент, ,ТипыДокументов); 
			Если КорректируемыеДокументыОпераций.Количество() = 0 Тогда
			//-бит
				ПараметрыФискализации.Результат = Ложь; 
			...
			//+бит Добавляем цикл чтобы обойти все фискальные операции
			ПараметрыФискализации.Вставить("ЧекиКоррекцииСторно", Новый Массив());
			Для Каждого КорректируемыйДокументОперация Из КорректируемыеДокументыОпераций Цикл
			//-бит	
				...
				//+бит
				// По какой-то причине в БПО невозможно задать Персональные данные для коррекции, добавляем
				Если ПараметрыОперации.ЕстьПерсональныеДанные Тогда
					ПараметрыФискализации.ЕстьПерсональныеДанные = ПараметрыОперации.ЕстьПерсональныеДанные;
					Если ПустаяСтрока(ПараметрыФискализации.ДатаВремяЧека) Тогда
						ДатаВремяЧека = МенеджерОборудованияВызовСервера.ДатаСеанса();
					Иначе
					...
					ПараметрыФискализации.ТипПерсональныхДанных = ПараметрыОперации.ТипПерсональныхДанных;
					ПараметрыФискализации.СубъектПерсональныхДанных = ПараметрыОперации.СубъектПерсональныхДанных;
					// При формировании чека с персональными данными не передается тип расчета, что приводит 
					// к его отсутсвию при записи в Фискальные операции
					ПараметрыФискализации.ТипРасчета = ПараметрыОперации.ТипРасчета;
					МенеджерОборудованияВызовСервераПереопределяемый.ОбработкаЗаполненияПерсональныхДанных(СведенияОПокупателе, ПараметрыФискализации.СубъектПерсональныхДанных, 
						ПараметрыФискализации.ТипПерсональныхДанных, ДатаВремяЧека);	
					СформироватьXMLПакетДляФискализацияЧека(ПараметрыОперации, ПараметрыФискализации, Истина, СведенияОПокупателе, ПараметрыФискализации.ТипПерсональныхДанных);
				КонецЕсли;
			...
			//+бит 
				//Нужен для коррекции коррекции, чтобы сторнирующие чеки не попадали 
				// в следующий документ коррекции
				ПараметрыФискализацииСторно.бит_Сторно = Истина;
				// Для сторнирующих чеков принудительно отключаем вывод персональных данных 
				ПараметрыФискализацииСторно.ЕстьПерсональныеДанные = Ложь;
				ПараметрыФискализации.ЧекиКоррекцииСторно.Добавить(ПараметрыФискализацииСторно);		
			КонецЦикла;
			// Если два чека печатаем из Коррекции то при печати второго чека нет необходимости печатать
			// сторнирующие чеки
			Если ПараметрыОперации.Свойство("НеСторнировать") И ПараметрыОперации.НеСторнировать Тогда
				ПараметрыФискализации.ЧекиКоррекцииСторно = Новый Массив();
			Иначе
				// Удаляем сторнирующие чеки если уже были напечатаны, порядок печати в алгоритме не меняется поэтому
				// удаляем по принципу очереди
				МенеджерОборудованияВызовСервераПереопределяемый.ПроверитьУжеФискализированныеИУдалитьЧекиКоррекции(ПараметрыФискализации.ЧекиКоррекцииСторно, ПараметрыОперации.ДокументОснование); 	
			КонецЕсли;
			//-бит
		КонецЕсли;
...
Процедура ОбработатьДанныеОперации(ПараметрыПодключения, Команда, РезультатВыполнения, ДанныеОперации) Экспорт
	...
	ИначеЕсли Команда = "PrintReceiptCorrection" Тогда
		...
		//+бит
		МенеджерОборудованияВызовСервераПереопределяемый.ОбработатьДанныеОперацииКоррекции(ДанныеОперации);
		//-бит
		...
		Если ДанныеОперации.ЕстьПерсональныеДанные Тогда
			РезультатВыполнения.ДанныеЧекаXML = ДанныеОперации.ДанныеЧекаXMLПерсональныеДанные;		
		КонецЕсли;
		//-бит
...
Процедура ЗаписатьФискальнуюОперацию(ПараметрыФискализации) Экспорт
	...
	//+бит
	Если ПараметрыФискализации.ЕстьПерсональныеДанные Тогда
		НоваяСтрока.ДанныеXML = Новый ХранилищеЗначения(ПараметрыФискализации.ДанныеЧекаXMLПерсональныеДанные);	
	КонецЕсли;
	Если ПараметрыФискализации.Свойство("бит_Сторно") Тогда
		НоваяСтрока.бит_Сторно = ПараметрыФискализации.бит_Сторно;
	КонецЕсли;
	Если ПараметрыФискализации.Свойство("бит_ПодключаемоеОборудование") Тогда
		НоваяСтрока.бит_ПодключаемоеОборудование = ПараметрыФискализации.бит_ПодключаемоеОборудование;
	КонецЕсли;
	//-бит
...
Функция СрокХраненияОперацийОчередиЧековИзНабораКонстант(НаборКонстант) Экспорт
	Возврат НаборКонстант.СрокХраненияОперацийОчередиЧеков;
КонецФункции

Функция СрокХраненияФискальныхОперацийИзНабораКонстант(НаборКонстант) Экспорт
	Возврат НаборКонстант.СрокХраненияФискальныхОпераций;
КонецФункции
...
Процедура ЗаписатьСведенияОПокупателе(ЗаписьXML, ОбщиеПараметры, ПерсональныеДанные = Неопределено, ТипПерсональныхДанных = Неопределено);
	...
	//+бит
	РаботаСТорговымОборудованием.ФЛК_СведенияОПокупателе_ФД12(Покупатель, ПокупательИНН, СведенияОПокупателе, ПерсональныеДанные);
	//-бит
