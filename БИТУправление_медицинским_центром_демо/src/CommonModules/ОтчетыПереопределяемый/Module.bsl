...
Процедура ПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка) Экспорт
	...
	//+бит
	Форма.Элементы.бит_ОткрытьФормуСпискаВариантовОтчета.Видимость = ПравоДоступа("Редактирование", Метаданные.Справочники.ВариантыОтчетов);
	
	Попытка
		ОтчетОбъект = Форма.РеквизитФормыВЗначение("Отчет");
		ОтчетОбъект.ОпределитьНастройкиФормы(Форма, "", Форма.НастройкиОтчета);
	Исключение
	КонецПопытки;
	
	ГруппаУровеньГруппировки = Форма.Элементы.Добавить("ГруппаУровеньГруппировки", Тип("ГруппаФормы"), Форма.Элементы.ГруппаРаботаВТаблице);
	ГруппаУровеньГруппировки.Вид = ВидГруппыФормы.Подменю;
	ГруппаУровеньГруппировки.Картинка = БиблиотекаКартинок.Группировка;
	ГруппаУровеньГруппировки.Отображение = ОтображениеКнопки.Картинка;
	ГруппаУровеньГруппировки.Заголовок = "Свернуть до уровня группировки";
	ГруппаУровеньГруппировки.Подсказка = "Свернуть до уровня группировки";
	Форма.Элементы.Переместить(ГруппаУровеньГруппировки, Форма.Элементы.ГруппаРаботаВТаблице, Форма.Элементы.КомандыТабличногоДокумента);
	
	Для НомерУровня = 1 По 10 Цикл
		
		ИмяКоманды = "Уровень" + Строка(НомерУровня);
		
		Команда = Форма.Команды.Добавить(ИмяКоманды);
		Команда.Действие = "Подключаемый_бит_ИзменитьУровеньГруппировки";
		Команда.Заголовок = "Уровень "+Строка(НомерУровня);
		Команда.Отображение = ОтображениеКнопки.Текст;
		
		Форма.ПостоянныеКоманды.Добавить(ИмяКоманды);
		
		НоваяКнопка = Форма.Элементы.Добавить(ИмяКоманды, Тип("КнопкаФормы"), ГруппаУровеньГруппировки);
		      
		НоваяКнопка.Вид                        = ВидКнопкиФормы.КнопкаКоманднойПанели;
		НоваяКнопка.ИмяКоманды                 = ИмяКоманды;
		НоваяКнопка.Заголовок                  = Команда.Заголовок;
		
	КонецЦикла;
	ГруппаУровеньГруппировки.Видимость = Ложь;
