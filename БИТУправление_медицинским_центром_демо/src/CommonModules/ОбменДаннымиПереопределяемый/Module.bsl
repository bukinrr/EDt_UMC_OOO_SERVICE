...
//  Строка - префикс кодов и номеров объектов информационной базы по умолчанию.
//
...
//
// Параметры:
// 	ПланыОбменаПодсистемы - Массив - из Метаданные.ПланыОбмена.
...
Процедура ПолучитьПланыОбмена(ПланыОбменаПодсистемы) Экспорт
	...
	ПланыОбменаПодсистемы.Добавить(Метаданные.ПланыОбмена.ОбменСФилиалами);
	// _Демо конец примера
...
// 		Соответствует параметру ИмяСобытия метода глобального контекста ЗаписьЖурналаРегистрации.
//
// 	КоличествоПолученныхОбъектов - Число - 
...
Процедура РегистрацияИзмененийНачальнойВыгрузкиДанных(Знач Получатель, СтандартнаяОбработка, Отбор) Экспорт
	...
	// _Демо начало примера
	Если ТипЗнч(Получатель) = Тип("ПланОбменаСсылка.Полный") Тогда
...
// Обработчик вызывается после определения получателей.
// Параметры:
//  МассивУзловРезультат - Массив - узлы.
//  Объект - Произвольный
//  ИмяПланаОбмена - Строка
//  ЭтоРегистр - Булево.
//
Процедура ПослеОпределенияПолучателей(МассивУзловРезультат, Объект, ИмяПланаОбмена, ЭтоРегистр) Экспорт
	
	мсУзлыКУдалению = Новый Массив;
	Для Каждого элУзел Из МассивУзловРезультат Цикл		
		НастройкаО = ?(ЗначениеЗаполнено(элУзел.НастройкаОбмена), элУзел.НастройкаОбмена, Перечисления.НастройкиОбменаСФилиалами.ОтдаватьВсе);
		
		Если элУзел.НеВыгружатьМедДокументы
			 И (ТипЗнч(Объект) = Тип("ДокументОбъект.Прием")
				Или ТипЗнч(Объект) = Тип("РегистрСведенийНаборЗаписей.ЗначенияПараметровHTML")
				Или ТипЗнч(Объект) = Тип("РегистрСведенийНаборЗаписей.ПланыЛечения")
				Или ТипЗнч(Объект) = Тип("ДокументОбъект.Рецепт")
				Или ТипЗнч(Объект) = Тип("ДокументОбъект.ДействияНадАнализами"))
		Тогда
			мсУзлыКУдалению.Добавить(элУзел);
			Продолжить;
		КонецЕсли;
		
		Если НастройкаО = Перечисления.НастройкиОбменаСФилиалами.НеОтдаватьНичего Тогда
			// Не регистрируется в любом случае.
			мсУзлыКУдалению.Добавить(элУзел);
		ИначеЕсли НастройкаО = Перечисления.НастройкиОбменаСФилиалами.ОтдаватьОбъектыСТемЖеФилиалом Тогда
			
			ФилиалОбъекта = Справочники.Филиалы.ПустаяСсылка();
			// Исключения среди регистров
			Если ЭтоРегистр Тогда
				Если ТипЗнч(Объект) = Тип("РегистрСведенийНаборЗаписей.ЗначенияПараметровHTML") Тогда		
					Если Объект.Отбор.Документ.Использование И ЗначениеЗаполнено(Объект.Отбор.Документ.Значение) Тогда 
						ФилиалОбъекта = Объект.Отбор.Документ.Значение.Филиал;	
					КонецЕсли;
				ИначеЕсли ТипЗнч(Объект) = Тип("РегистрСведенийНаборЗаписей.ПланыЛечения") Тогда
					Если Объект.Отбор.ДокументРегистратор.Использование И ЗначениеЗаполнено(Объект.Отбор.ДокументРегистратор.Значение) Тогда 
						ФилиалОбъекта = Объект.Отбор.ДокументРегистратор.Значение.Филиал;	
					КонецЕсли;
				Иначе
					Продолжить;
				КонецЕсли;
			Иначе
				Попытка
					ФилиалОбъекта = Объект.Филиал;
				Исключение
					ФилиалОбъекта = элУзел.Филиал;
				КонецПопытки;
			КонецЕсли;
			
			Если ФилиалОбъекта <> элУзел.Филиал Тогда
				мсУзлыКУдалению.Добавить(элУзел);
			КонецЕсли;
	...
	Для Каждого элУзел Из мсУзлыКУдалению Цикл
		УзелКУдалению = МассивУзловРезультат.Найти(элУзел);
		Если УзелКУдалению <> Неопределено Тогда
			МассивУзловРезультат.Удалить(УзелКУдалению);
		КонецЕсли;
	КонецЦикла;
