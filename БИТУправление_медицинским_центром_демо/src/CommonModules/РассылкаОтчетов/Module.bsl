...
Процедура ДобавитьЭлементВТаблицуТиповПолучателей(ТаблицаТипов, ДоступныеТипы, Настройки) Экспорт
	...
	Если Настройки.Свойство("ВидКИ") Тогда//И Не Настройки.ВидКИ.ЭтоГруппа Тогда
...
Функция СформироватьСписокПолучателейРассылки(ПараметрыЖурнала, Рассылка) Экспорт
	...
	Если ТипПолучателей = Тип("СправочникСсылка.Пользователи") Тогда
		...
		|Где НЕ ТаблицаПолучателей.Исключен
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗРЕШЕННЫЕ РАЗЛИЧНЫЕ
		|	втПолучатели.Получатель КАК Получатель,
		|	Выразить(тчКонтактнаяИнформация.Представление как Строка(1000)) КАК EMail
		|ИЗ
		|	втТаблицаПолучателей КАК втПолучатели
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КонтактнаяИнформация КАК тчКонтактнаяИнформация
		|		ПО втПолучатели.Получатель = тчКонтактнаяИнформация.Объект
		|ГДЕ
...
Процедура СформироватьИСохранитьОтчет(ПараметрыЖурнала, ПараметрыОтчета, ДеревоОтчетов, ПараметрыДоставки, ПолучательСсылка)
	...
	//+бит
	ПредставлениеОтчета = ПредставлениеОтчета + " (" + СтрокаОтчет.Настройки.Метаданные.Синоним + ")";
	//-бит
	...
	Для Каждого Формат Из ПараметрыОтчета.Форматы Цикл
		...
		ПолноеИмяФайла = КаталогПолучателя + ПривестиИмяФайла(
			ПредставлениеОтчета + //+бит СтрокаОтчет.Ключ + 
