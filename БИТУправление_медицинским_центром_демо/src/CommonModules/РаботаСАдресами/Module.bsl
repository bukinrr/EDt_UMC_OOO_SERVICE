...
// Событие перед записью XDTO-объекта контактная информация.
//
// Параметры:
//  ТекстXML - Строка	 - исходный текст адреса.
// 
// Возвращаемое значение:
//  Строка. 
//
...
// Функция - Дополнительные правила преобразования
// 
// Возвращаемое значение:
//  Строка.
//
...
// Обработка частых сокращений в адресах
//
// Параметры:
//  ДанныеАнализа	 - Массив	 - части адреса.
//
...
// Добавляет формат контактной информации
//
// Параметры:
//  ФорматКонтактнойИнформации	 - Структура	 - структура адреса.
//
...
Функция КонвертироватьАдресИзJSONВXML(Знач ЗначенияПолей, Знач Представление, Знач ОжидаемыйТип = Неопределено) Экспорт
	...
	Если ТипЗнч(ЗначенияПолей) = Тип("Структура")
		И ЗначенияПолей.Свойство("КраткоеПредставление")
	Тогда
		Попытка
			Результат.Состав.Состав.КраткоеПредставление = ЗначенияПолей.КраткоеПредставление;
		Исключение
		КонецПопытки;
	КонецЕсли;
	//-БИТ
