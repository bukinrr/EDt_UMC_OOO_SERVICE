...
Процедура ПриОпределенииНастроекИнтеграции(Настройки) Экспорт
	...
	Настройки.ОбъектМетаданных = Метаданные.РегистрыСведений.СоответствиеНастроекСБП;
...
Процедура ПриЗаписиНастроекИнтеграции(ПараметрыОплаты, Отказ, СообщениеОбОшибке) Экспорт
	...
	Попытка
		...
		ЭлементБлокировки = Блокировка.Добавить("РегистрСведений.СоответствиеНастроекСБП");
		ЭлементБлокировки.УстановитьЗначение("ТорговаяТочка", ПараметрыОплаты.Получить("ТорговаяТочка"));
		ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
		Блокировка.Заблокировать();
		
		Набор = РегистрыСведений.СоответствиеНастроекСБП.СоздатьНаборЗаписей();
		Набор.Отбор.ТорговаяТочка.Установить(ПараметрыОплаты.Получить("ТорговаяТочка"));
		Набор.Записать();
				
		МЗ = РегистрыСведений.СоответствиеНастроекСБП.СоздатьМенеджерЗаписи();
		МЗ.ТорговаяТочка = ПараметрыОплаты.Получить("ТорговаяТочка");
		МЗ.НазначениеПлатежа = ПараметрыОплаты.Получить("НазначениеПлатежа");
		СрокЖизниQRКода = ПараметрыОплаты.Получить("СрокЖизниQRКода");
		Если Не ЗначениеЗаполнено(СрокЖизниQRКода) Тогда
			СрокЖизниQRКода = 129600;
		КонецЕсли;
		МЗ.СрокЖизниQRКода = СрокЖизниQRКода; 
		МЗ.Записать();
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			...
			Метаданные.РегистрыСведений.СоответствиеНастроекСБП,
			,
...
Процедура ПриНастройкеЭлементовФормыИнтеграции(НастройкиФормы, ДополнительныеПараметры) Экспорт
	...
	НастройкиФормы.ЭлементыНастроекОплаты.НазначениеПлатежа.Подсказка
		= НСтр("ru = 'Позволяет указать произвольную строку, которую увидет клиент при оплате через СБП на своем устройстве. 
		|Стандартный вариант строки: ""Оплата СБП <СуммаОплаты> RUB"". Так же чтобы вывести в произвольной строке сумму текущей оплаты можно использовать ""<СуммаОплаты>"" в произвольном месте. 
		|Cтрока не должна превышать 140 символов.'");
		
	НастройкиФормы.ЭлементыНастроекОплаты.НазначениеПлатежа.ОтображениеПодсказки
		= ОтображениеПодсказки.ОтображатьСнизу;
		
		
	НастройкиФормы.ЭлементыНастроекОплаты.СрокЖизниQRКода.Подсказка
		= НСтр("ru = 'Может принимать значения от 5 минут до 129 600 минут (90 дней), если не указывать срок действия будет установлен в значение 90 дней.'");
		
	НастройкиФормы.ЭлементыНастроекОплаты.СрокЖизниQRКода.ОтображениеПодсказки
		= ОтображениеПодсказки.ОтображатьСнизу;
		
	НастройкиФормы.ОбщиеЭлементы.Наименование.СписокВыбора.Очистить();
...
Процедура ПриЗаполненииФормыИнтеграции(Настройки, ДополнительныеПараметры) Экспорт
	...
	//	Настройки.НастройкиОплаты.Филиал = ДополнительныеПараметры.Филиал;
	//КонецЕсли;
	////Настройки.НастройкиОплаты.СпособОплаты = Перечисления._ДемоСпособыОплаты.СистемаБыстрыхПлатежей;
	//
	//// _Демо конец примера
...
Процедура ПриФормированииЗаказаНаОплатуСБП(
	...
	Если ТипЗнч(ДокументОплаты) = Тип("ДокументСсылка.ОказаниеУслуг") Тогда
		Отбор = Новый Структура("ВидОплаты",Перечисления.ВидыОплаты.Безналичные);
		мОплатыБезналом = ДокументОплаты.Оплата.НайтиСтроки(Отбор);		
		Если мОплатыБезналом.Количество() = 1 Тогда	
			ЗаказНаОплату.СуммаОплаты = мОплатыБезналом[0].Сумма;
			ЗаказНаОплату.ДатаОплаты = ДокументОплаты.Дата;
			ЗаказНаОплату.СрокЖизниQRКода = 129600;
		Иначе
			ВызватьИсключение "Количество строк оплаты безналичным более 1 или отсутствует!";	
		КонецЕсли;
	ИначеЕсли ТипЗнч(ДокументОплаты) = Тип("ДокументСсылка.ОплатаПлатежнойКартой")  Тогда
		РеквизитыДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
			ДокументОплаты,
			"СуммаДокумента, Дата");
		ЗаказНаОплату.СуммаОплаты = РеквизитыДокумента.СуммаДокумента;
		ЗаказНаОплату.ДатаОплаты = РеквизитыДокумента.Дата;
		//ЗаказНаОплату.ОтложенноеПолучениеСтатуса = Истина;
	Иначе
		// Не должно прийти сюда но на всякий случай
		ВызватьИсключение "Выбран не допустимый тип документов";
	КонецЕсли;
	
	НастройкиТорговойТочки = ИнтеграцияСБП.ПолучитьНастройкиТорговойТочки(ТорговаяТочка);
	Если ЗначениеЗаполнено(НастройкиТорговойТочки.СрокЖизниQRКода) Тогда
		ЗаказНаОплату.СрокЖизниQRКода = НастройкиТорговойТочки.СрокЖизниQRКода;
	КонецЕсли;
	Если ЗначениеЗаполнено(НастройкиТорговойТочки.НазначениеПлатежа) Тогда
		ЗаказНаОплату.НазначениеПлатежа = СтрЗаменить(НастройкиТорговойТочки.НазначениеПлатежа, "<СуммаОплаты>", Строка(Формат(ЗаказНаОплату.СуммаОплаты, "ЧГ=0")));		
	КонецЕсли;
...
Процедура ПриФормированииЗаказаНаВозвратСБП(
		...
		Если ТипЗнч(ДокументВозврата) = Тип("ДокументСсылка.ОказаниеУслуг") Тогда
			Отбор = Новый Структура("ВидОплаты",Перечисления.ВидыОплаты.Безналичные);
			мОплатыБезналом = ДокументВозврата.Оплата.НайтиСтроки(Отбор);		
			Если мОплатыБезналом.Количество() = 1 Тогда	
				ЗаказНаВозврат.СуммаВозврата = мОплатыБезналом[0].Сумма;
				ЗаказНаВозврат.ДатаВозврата = ДокументВозврата.Дата;
			Иначе
				ВызватьИсключение "Количество строк оплаты безналичным более 1 или отсутствует!";	
			КонецЕсли;
		ИначеЕсли ТипЗнч(ДокументВозврата) = Тип("ДокументСсылка.ВозвратПоПлатежнойКарте") Тогда
			РеквизитыДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
			ДокументВозврата,
			"СуммаДокумента, Дата");
			ЗаказНаВозврат.СуммаВозврата = ДокументВозврата.СуммаДокумента;
			ЗаказНаВозврат.ДатаВозврата = ДокументВозврата.Дата;
			//ЗаказНаВозврат.ОтложенноеПолучениеСтатуса = Истина;
		Иначе
			// Не должно прийти сюда но на всякий случай
			ВызватьИсключение "Выбран не допустимый тип документов";
		КонецЕсли;
...
Процедура ПриЗагрузкеСтатусаОперации(
	...
	СтатусОплаты = Перечисления.СтатусыОперацияСБП.Ошибка;
	Если РезультатОбработки.СтатусОперации = ИнтеграцияСПлатежнымиСистемамиКлиентСервер.СтатусОперацииВыполнена() Тогда
		Статус = Перечисления.СтатусыОперацияСБП.Выполнена;
	ИначеЕсли РезультатОбработки.СтатусОперации = ИнтеграцияСПлатежнымиСистемамиКлиентСервер.СтатусОперацииОтменена() Тогда
		Статус = Перечисления.СтатусыОперацияСБП.Отменена;
	ИначеЕсли РезультатОбработки.СтатусОперации = ИнтеграцияСПлатежнымиСистемамиКлиентСервер.СтатусОперацииОшибка() Тогда
		Статус = Перечисления.СтатусыОперацияСБП.Ошибка;
	КонецЕсли;
