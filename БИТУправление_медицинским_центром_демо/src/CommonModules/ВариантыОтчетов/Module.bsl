...
Функция ОтчетыТекущегоПользователя() Экспорт
	...
	ВариантыОтчетовПереопределяемый.ПриДобавленииДополнительныхОтчетовДоступныхТекущемуПользователю(ДоступныеОтчеты);
...
Процедура СброситьПользовательскиеНастройки(Ключ, ТипыНастроек = Неопределено) Экспорт
	...
	|	ЕСТЬNULL(ВариантыОтчетов.Отчет.Имя, """") КАК ИмяОтчета,
...
Процедура ПеренестиПользовательскиеИзСтандартногоХранилища(ИменаОтчетов = "") Экспорт
	...
	Пока Истина Цикл
		...
		//+бит
		Пользователь = Справочники.Пользователи.НайтиПоКоду(ВыборкаХранилища.Пользователь);
		...
		СтрокаТаблицы.Автор     = Пользователь;
...
Процедура ЗагрузитьПользовательские(ТаблицаВариантов = Неопределено) Экспорт
	...
	|ГДЕ
	|	Пользователи.Ссылка В(&Идентификаторы)
	|	И Пользователи.ПометкаУдаления = ЛОЖЬ";
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Идентификаторы", ТаблицаВариантов.ВыгрузитьКолонку("Автор"));
	Запрос.Текст = ТекстЗапроса;
	
	ПользователиПоСсылке = Запрос.Выполнить().Выгрузить();
	...
	Для Каждого СтрокаТаблицы Из ОпределенныеВарианты Цикл
		...
		// Автор варианта
		ПользовательПоИД = ПользователиПоСсылке.Найти(СтрокаТаблицы.Автор, "Пользователь");
	...
	Если НадоПрочитатьСсылкуОтчета Или НадоПрочитатьНастройки Тогда
		Если ТипЗнч(Результат.СсылкаВарианта) <> Тип("СправочникСсылка.ВариантыОтчетов")
			...
			Если Результат.СсылкаВарианта = Неопределено Тогда
				...
				//+Бит. Принудительное обновление списка вариантов отчетов.
				Попытка
					УстановитьПривилегированныйРежим(Истина);
					ОбновитьПовторноИспользуемыеЗначения();
					БИТ_ОбновлениеИнформационнойБазы.ОбновитьДанныеВариантовОтчетов();
					УстановитьПривилегированныйРежим(Ложь);
					Результат.ТекстОшибки = Результат.ТекстОшибки + НСтр("ru=' Попробуйте закрыть отчет и открыть заново.'");
				Исключение
				КонецПопытки;
				//-Бит.
				Возврат Результат;
...
Функция ДоступностьОтчетов(СсылкиОтчетов) Экспорт
	...
	Для Каждого Ссылка Из СсылкиОтчетов Цикл
		...
		Если ТипЗнч(Ссылка) = Тип("СправочникСсылка.ВариантыОтчетов") Тогда
			СтрокаТаблицы.Представление = СтрокаТаблицы.Представление + " (" + Строка(Ссылка.Отчет) + ")";
		КонецЕсли;
		//-бит
		Тип = ТипЗнч(Ссылка);
...
Функция СформироватьИнформациюОбОтчетеПоПолномуИмени(ОтчетПолноеИмя) Экспорт
	...
	Если ВРег(Префикс) = "ОТЧЕТ" или Префикс = "" Тогда
...
Процедура СформироватьОтчетВФоне(Параметры, АдресХранилища) Экспорт
	...
	Попытка
		...
		ПодключенныеРасширения = ПараметрыСеанса.ПодключенныеРасширения;
	Исключение
...
Функция НаименованиеЗанято(Отчет, Ссылка, Наименование) Экспорт
	...
	НаименованиеЗанято = Ложь;
	
	Если РезультатЗапроса.Следующий() Тогда
		НаименованиеЗанято = Истина;
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Отчет, "ДополнительныеСвойства")
			И Отчет.ДополнительныеСвойства.Свойство("ОбновлениеИБ")
		Тогда 
			ВариантОбъект = РезультатЗапроса.Ссылка.ПолучитьОбъект();
			Если РезультатЗапроса.Пользовательский Тогда
				ВариантОбъект.Наименование = ВариантОбъект.Наименование + "(польз.)";
				Попытка
					ВариантОбъект.Записать();
					НаименованиеЗанято = Ложь;
				Исключение
...
Функция ОперативноеОбновлениеОбщихДанных(Режим, РазделенныеОбработчики) Экспорт
...
Процедура ОбновитьИндексПоиска(Кэш)
	...
	Если Кэш.ОбщиеДанные Тогда
		...
		ИначеЕсли Кэш.Режим = "ОбщиеДанныеРасширений" Тогда
			...
			Запрос.УстановитьПараметр("ВерсияРасширений", "0");
...
Функция ОбновитьПредопределенный(Кэш, ОписаниеВарианта)
	...
	ВариантОбъект.Описание = ВариантОбъект.Отчет;
...
Процедура ОбновитьРазделенныйПредопределенный(Кэш, Шаблоны, СтрокаТаблицы)
	...
	ВариантОбъект.Описание = ВариантОбъект.Отчет;
	ВариантОбъект.Заблокировать();
	ВариантОбъект.ДополнительныеСвойства.Вставить("ОбновлениеИБ", Истина);
	ВариантОбъект.Записать();
...
Функция НайтиСсылки(ПараметрыПоиска) Экспорт
	...
	Иначе
		...
		|	ВЫРАЗИТЬ(ВариантыОтчетов.Описание КАК СТРОКА(1000)) КАК Поле1,
		|	ВариантыОтчетов.ТолькоДляАвтора КАК ТолькоДляАвтора,
		|	ВариантыОтчетов.Автор КАК Автор
		|ИЗ
		|	Справочник.ВариантыОтчетов КАК ВариантыОтчетов
		...
		|
		|ВЫБРАТЬ
		|	ПредопределенныеРасширения.Ссылка,
		|	ПредопределенныеРасширения.КлючВарианта,
		|	ПредопределенныеРасширения.Родитель,
		|	ПредопределенныеРасширения.Наименование,
		|	ВЫРАЗИТЬ(ПредопределенныеРасширения.Описание КАК СТРОКА(1000)),
		|	ЛОЖЬ,
		|	НЕОПРЕДЕЛЕНО
		|ИЗ
		|	Справочник.ПредопределенныеВариантыОтчетовРасширений КАК ПредопределенныеРасширения
		|ГДЕ
		|	НЕ ПредопределенныеРасширения.ПометкаУдаления
		|	И ПредопределенныеРасширения.Отчет В (&ОтчетыПользователя)";  
