...
Функция СоздатьКоллекциюКомандПечати() Экспорт
	...
	// описание
	//+БИТ
	МсОписаниеТипов =  Новый Массив;
	МсОписаниеТипов.Добавить(Тип("Строка"));
	МсОписаниеТипов.Добавить(Тип("СправочникСсылка.ВнешниеОбработки"));
	МсОписаниеТипов.Добавить(Тип("СправочникСсылка.ПакетыПечати"));
	МсОписаниеТипов.Добавить(Тип("СправочникСсылка.ШаблоныПечати"));
	//-БИТ
	Результат.Колонки.Добавить("Идентификатор", Новый ОписаниеТипов(МсОписаниеТипов));
...
Функция СформироватьПечатныеФормы(Знач ИмяМенеджераПечати, Знач ИменаМакетов, Знач МассивОбъектов, Знач ПараметрыПечати, 
	...
	Если ТипЗнч(ИменаМакетов) = Тип("Строка") Тогда
		ИменаМакетов = СтрРазделить(ИменаМакетов, ",");
	//+БИТ
	ИначеЕсли ТипЗнч(ИменаМакетов) = Тип("СправочникСсылка.ВнешниеОбработки") Тогда
	     ИменаМакетов = Строка(ИменаМакетов);
	//-БИТ
	Иначе // Тип("Массив")
...
Функция СформироватьПечатныеФормыДляБыстройПечати(ИмяМенеджераПечати, ИменаМакетов, МассивОбъектов, ПараметрыПечати) Экспорт
	...
	Если НЕ ПравоДоступа("Вывод", Метаданные) Тогда
		...
		Возврат Результат;
	КонецЕсли;
	ИменаМакетов = СтрЗаменить(ИменаМакетов,"_принтер","");
...
//+бит.Взята из БПО, т.к. в БСП отдельная менее надежная компонента.
Функция КомпонентаФормированияQRКода(Отказ)
	...
	Попытка
		Если ПодключитьВнешнююКомпоненту("ОбщийМакет.КомпонентаПечатиQRКода", "QR",,ТипПодключенияВнешнейКомпоненты.НеИзолированно) Тогда
...
Функция ПакетСОднимТабличнымДокументом(ТабличныйДокумент)
	...
	//+БИТ Совместно с УправлениеПечатьюКлиент.РаспечататьТабличныеДокументы() дает более простой выбор неоснвного принтера для печати.
	// ЗаполнитьЗначенияСвойств(ПакетСОднимДокументом, ТабличныйДокумент, "Вывод, ДвусторонняяПечать, ИмяПринтера, КоличествоЭкземпляров, ТочностьПечати"); // Исходный код из БСП.
	КопируемыеСвойства = "Вывод, ДвусторонняяПечать, КоличествоЭкземпляров, ТочностьПечати"; // Т.е. без имени принтера.
	Если Не ПустаяСтрока(ТабличныйДокумент.ИмяПринтера) Тогда
		КопируемыеСвойства = КопируемыеСвойства + ", ИмяПринтера";
	КонецЕсли;
	ЗаполнитьЗначенияСвойств(ПакетСОднимДокументом, ТабличныйДокумент, КопируемыеСвойства);
	//-БИТ
...
Процедура ПриПолученииКомандПечатиОбъекта(КомандыПечати, СведенияОбОбъекте, ДобавлятьДополнительные)
	УправлениеПечатьюПереопределяемый.ПриПолученииКомандПечатиОбъекта(КомандыПечати, СведенияОбОбъекте, ДобавлятьДополнительные);
...
//
Функция ИнициализироватьПечатнуюФорму(Знач УдалитьТипДокумента = Неопределено, Знач УдалитьНастройкиСтраницыМакета = Неопределено, Макет = Неопределено) Экспорт
