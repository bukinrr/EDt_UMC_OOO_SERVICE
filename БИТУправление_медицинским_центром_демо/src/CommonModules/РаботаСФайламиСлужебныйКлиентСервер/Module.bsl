...
Процедура ЗаполнитьСтатусПодписи(СтрокаПодписи) Экспорт
	...
	// +БИТ
	Попытка
		СтрокаПодписи.Статус = бит_СертификатыЭЦПКлиентСервер.ПолучитьОбщийСтатусПроверкиПодписи(
					СтрокаПодписи.ПодписьВерна, СтрокаПодписи.СертификатДействителен, СтрокаПодписи.ДатаПроверкиПодписи);
		Возврат;
