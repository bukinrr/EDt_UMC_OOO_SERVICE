...
//
// Параметры:
//  ИдентификаторРегиона - Строка - регион
//
// Возвращаемое значение:
...
// Параметры:
//  Адрес - Строка
//  Источник - Строка - Источник адреса.
//
...
// Процедура УстановитьМуниципальныеСведения.
//
// Параметры:
//  Адрес	 - Строка	 - адрес.
//  ОКТМО	 - Строка	 - ОКТМО.
...
// Функция РаспознатьАдрес.
//
// Параметры:
//  ЧастиАдреса - Строка
//  Представление - Строка
//  ВСтруктуру - Булево - По умолчанию Ложь
//
// Возвращаемое значение:
//  Строка.
//
...
Функция АктуальныеАдресныеСведения(СведенияОбАдресномОбъекте) Экспорт
	...
	Иначе
		...
		ЗагруженныеДанные = ЗагруженныеАдресныеСведения["ИспользоватьЗагруженные"];
	КонецЕсли;
...
// Процедура Обработчик. = "АдресныйКлассификаторСлужебный.ВыполнитьНачальноеЗаполнение";	Обработчик.НачальноеЗаполнение = Истина;	Обработчик.ОбщиеДанные         = Истина;	Обработчик.РежимВыполнения = "Оперативно";	КонецПроцедуры// См. ТекущиеДелаПереопределяемый.ПриОпределенииОбработчиковТекущихДел. ПриЗаполненииСпискаТекущихДел.
//
...
// +бит
// Параметры:
//  XDTOАдрес - Неопределено
//  ДляПостроенияСтруктурыПолей - Булево - По умолчанию Истина
//
// Возвращаемое значение:
//  Неопределено.
//
Функция ДопЭлементыАдреса(XDTOАдрес, ДляПостроенияСтруктурыПолей = Истина) Экспорт
	...
	Для Каждого ДопЭлемент Из XDTOАдрес.ДопАдрЭл Цикл
		Если ДопЭлемент.Номер <> Неопределено Тогда
			...
			Если ТипОбъекта <> Неопределено Тогда
				Если ДляПостроенияСтруктурыПолей
					И ТипОбъекта.Тип >= 1 И ТипОбъекта.Тип <= 3
				Тогда
				    Если ТипОбъекта.Тип = 1 Тогда
						ВнутреннееИмяПоляАдреса = "Дом";
					ИначеЕсли ТипОбъекта.Тип = 2 Тогда
						ВнутреннееИмяПоляАдреса = "Корпус";
					ИначеЕсли ТипОбъекта.Тип = 3 Тогда
						ВнутреннееИмяПоляАдреса = "Квартира";
					КонецЕсли;
					
					Результат.Вставить(ВнутреннееИмяПоляАдреса,		  ДопЭлемент.Номер.Значение);
				Иначе
...
// Функция ОпределитьИдентификаторыАдреса.
//
// Параметры:
//  АдресДляПроверки - Неопределено
//
// Возвращаемое значение:
//  Неопределено.
//
...
Процедура ЗаполнитьАктуальныеАдресныеСведенияВнутр(Результат, Значение)
	...
	//+бит
	Индекс = 5;
	ЕстьСвойствоМуниципальный = ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Значение, "Муниципальный");
	
	Пока Индекс >= 0 И (Не ЕстьСвойствоМуниципальный Или Значение.Муниципальный) Цикл
		ЗаполнитьПоляВСоответствииСУровнемАдреса(Результат, Выборка[0], Индекс, СловарьИменПолей, ЭтоМуниципальныйАдрес(Выборка[0].ТипАдреса));
		Индекс = Индекс - 1;
	КонецЦикла;
	//-бит
...
Процедура ЗаполнитьСписокАвтоподбораЧастиАдресаВнутр(Результат, КешПредставлений, Знач Текст, ДополнительныеПараметры, ЗагруженныеАдресныеСведения)
	...
	Для Каждого Строка Из РезультатЗапроса.Выгрузить() Цикл
		...
		Если Строка.Уровень = 10 И Не ЗагруженныеАдресныеСведения.Получить("КлассификаторДоступен") Тогда
			...
			Если ТипЗнч(ЗагруженныеАдресныеСведения.Получить(КодРегиона)) = Тип("Структура")
				И ЗагруженныеАдресныеСведения.Получить(КодРегиона).ИспользоватьЗагруженные = Ложь
			Тогда
...
Процедура ЗаполнитьВерсиюПоставщикаДанныхСервис1С(Результат)
	...
	//+БИТ Не требуется подгрузка на ходу из веб-сервиса 1С. И тормозит при недоступности.
...
//       * ИдентификаторАдресногоОбъекта - УникальныйИдентификатор - Идентификатор адресного объекта (улицы, нас.
//                                                                   пункта).
...
Функция АнализОдногоАдресаПоКлассификатору(Адрес)
	...
	Если ПустаяСтрока(НомерДомаАдрес) И Адрес.Buildings.Количество() = 0 Тогда
		// У адреса нет домов, корпусов и строений. Возможно это почтовый адреса с а/я или до востребования.
		Возврат Результат;
	КонецЕсли;
...
//
// Параметры:
//  ПараметрыВыгрузки - Неопределено - По умолчанию Неопределено
//  АдресВоВременномХранилище - Неопределено - По умолчанию Неопределено
//
// Возвращаемое значение:
//  Неопределено.
//
...
// Функция АдаптацияПолейКонтактнойИнформации.
//
// Параметры:
//  Свойство - Неопределено
//  Значение - Неопределено
//  ДополнительныеПараметрыФункцииПреобразования - Неопределено
//  Отказ - Булево
//
// Возвращаемое значение:
//  Неопределено.
...
// Функция ВосстановлениеПолейКонтактнойИнформации.
//
// Параметры:
//  Свойство - Неопределено
//  Значение - Неопределено
