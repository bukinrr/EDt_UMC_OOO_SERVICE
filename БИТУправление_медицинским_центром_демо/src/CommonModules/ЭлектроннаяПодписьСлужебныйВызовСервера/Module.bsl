...
Функция ВыполнитьНаСторонеСервера(Знач Параметры, АдресРезультата, ОперацияНачалась, ОшибкаНаСервере) Экспорт
	...
	// ПараметрыСоздания.ОписаниеОшибки = Новый Структура; //+бит убрано
	
	МенеджерКриптографии = ЭлектроннаяПодписьСлужебный.МенеджерКриптографии(Параметры.Операция, ПараметрыСоздания);
	
	//+бит ПараметрыСоздания.ОписаниеОшибки может быть уже передан и не быть структурой (быть строкой).
	// ОшибкаНаСервере = ПараметрыСоздания.ОписаниеОшибки;
	Если Не ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ПараметрыСоздания, "ОписаниеОшибки") Тогда
		ПараметрыСоздания.ОписаниеОшибки = Новый Структура;
	КонецЕсли;
	
	Если ТипЗнч(ПараметрыСоздания.ОписаниеОшибки) = Тип("Структура") Тогда
		ОшибкаНаСервере = ПараметрыСоздания.ОписаниеОшибки;
	ИначеЕсли ЗначениеЗаполнено(ПараметрыСоздания.ОписаниеОшибки) Тогда
		ОшибкаНаСервере.Вставить("ОписаниеОшибки", ПараметрыСоздания.ОписаниеОшибки);
	КонецЕсли;
	//-бит
