...
//  КлючНастроек - Строка - имя настройки
...
// Параметры:
//  Знач ПроверитьОтсутствиеДругихСеансов - Булево - По умолчанию Истина
//
...
// Параметры:
//  Использование - Булеов - режим.
//  ОбластьДанных - Строка - область разделения.
//
...
// Параметры:
//  Знач ВключитьРазделение - Булево - По умолчанию Ложь
//
...
// Параметры:
//  ОбъектВспомогательныхДанных - Произвольный - объект.
//
...
//                                если какой-либо объект будет исключен из выборки по правам, то все реквизиты этого
//                                объекта вернутся со значением Неопределено;
...
// Функция ОбщиеПараметрыБазовойФункциональности.
//
// Возвращаемое значение:
//  Структура.
//
Функция ОбщиеПараметрыБазовойФункциональности() Экспорт
	...
	ОбщиеПараметры.Вставить("МинимальнаяВерсияПлатформы", "8.3.8.0");
...
Функция РазделениеВключено() Экспорт
	...
	Если ПодсистемаСуществует("СтандартныеПодсистемы.РаботаВМоделиСервиса") Тогда
		МодульРаботаВМоделиСервиса = ОбщийМодуль("ОбщегоНазначенияПовтИсп");
...
//  Булево - Истина, если есть.
//
Функция ЕстьРеквизитОбъектаОбратныйПорядок(ИмяРеквизита, МетаданныеОбъекта) Экспорт
...
//  Булево - Истина, если есть.
//
Функция ЕстьРеквизитВМетаданныхОбъекта(МетаданныеОбъекта, ИмяРеквизита) Экспорт
...
//  Булево - Истина, если есть.
//
Функция ЕстьРеквизитОбъектаПоМетаданным(МетаданныеОбъекта, ИмяРеквизита) Экспорт
...
Функция ПодключитьКомпонентуИзМакета(Идентификатор, ПолноеИмяМакета) Экспорт
	...
	#Если ВебКлиент Или МобильныйКлиент Тогда
		ТипПодключенияКомпоненты = Неопределено;
	#Иначе
		ТипПодключенияКомпоненты = ТипПодключенияВнешнейКомпоненты.НеИзолированно;
	#КонецЕсли
	Если ПодключитьВнешнююКомпоненту(Местоположение, СимволическоеИмя,, ТипПодключенияКомпоненты) Тогда
	//-бит
...
//
// 	  * ВариантРаботыИнформационнойБазы             - Число -  Вариант работы информационной базы 
//																0 - файловый 1 - клиент-серверный
...
//                            то подробное описание ошибки помещается в этот параметр. По умолчанию ""
//	ОшибкаПодключенияКомпоненты - Булево - По умолчанию Ложь
//
// Возвращаемое значение:
//  COM-объект, Неопределенно -
...
//
// Параметры:
//  СобытияДляЖурналаРегистрации - Массив	 - клиентская глобальная переменная. Каждая структура - сообщение для журнала регистрации. После записи переменная очищается.
//
...
//
// Параметры:
//  СписокУровней	 - СписокЗначений	 - Наименования уровней регистрации событий, которые необходимо включить.
//
...
//
// Параметры: 
// 	СписокПроверок - СписокЗначений, список строк-представлений режимов использования журнала регистрации, которые надо
...
// Функция ЗначениеПеречисленияПоИмени.
//
// Параметры:
//  Имя - Строка
...
//  ПеречислениеСсылка.
//
Функция ЗначениеПеречисленияПоИмени(Имя, ИмяПеречисления) Экспорт
	
	ПеречислениеМенеджер = Перечисления[ИмяПеречисления];
	ОбъектМетаданных = ПеречислениеМенеджер.ПустаяСсылка().Метаданные();
	
	ИндексЗначения = ОбъектМетаданных.ЗначенияПеречисления.Индекс(ОбъектМетаданных.ЗначенияПеречисления.Найти(Имя));
	
	Если ИндексЗначения = -1 Тогда
		Возврат ПеречислениеМенеджер.ПустаяСсылка();
	Иначе
		Возврат ПеречислениеМенеджер.Получить(ИндексЗначения);
	КонецЕсли;	
...
//
// Параметры:
//  ИмяЭкспортнойПроцедуры	 - Строка	 - имя экспортной процедуры в формате <имя объекта>.<имя процедуры>, где <имя объекта> - это общий модуль или модуль менеджера объекта
//  Параметры				 - Структура - Параметры
//
Процедура ВыполнитьБезопасно(Знач ИмяЭкспортнойПроцедуры, Знач Параметры = Неопределено) Экспорт
...
//
// Параметры:
//  Отбор	 - Структура - Отбор, который требуется заполнить значениями
...
Функция СформироватьСуммуПрописью(Сумма, знач Валюта) Экспорт
	...
	Если ЗначениеЗаполнено(Валюта) Тогда
		Если Валюта.ПараметрыПрописиНаРусском = "" Тогда
			Возврат ОбщегоНазначенияКлиентСервер.ФорматСумм(Сумма);
		Иначе
			Возврат ЧислоПрописью(Сумма, "L=ru_RU", Валюта.ПараметрыПрописиНаРусском);
		КонецЕсли;
	Иначе             
		Возврат ЧислоПрописью(Сумма, "L=ru_RU", "рубль, рубля, рублей, м, копейка, копейки, копеек, ж, 2");
	КонецЕсли;
...
//  НазваниеДокумента	 - Строка	 - По умолчанию ""
// 
// Возвращаемое значение:
...
//
// Возвращаемое значение:
//  Булево - Истина - нашли с таким именем, Ложь - не нашли.
//
...
//
// Параметры:
//  ИмяТабЧасти			 - Строка		 - строковое имя искомой табличной части,
...
//  МетаданныеДокумента	 - Метаданные	 - объект описания метаданных документа, среди реквизитов которого производится поиск.
//  ИмяТабЧасти			 - Строка		 - строковое имя табличной части документа, среди реквизитов которого производится поиск.
// 
// Возвращаемое значение:
...
// Параметры:
//  ИмяРеквизита	 - Строка		 - строковое имя искомого реквизита,
//  МетаданныеОтчета - Метаданные	 - метаданные отчета, среди реквизитов которого производится поиск.
// 
// Возвращаемое значение:
//  Булево - Истина - нашли реквизит с таким именем, Ложь - не нашли.
//
...
//
// Параметры:
//  СтрокаПроверки - Строка - Строка для проверки только цифр.
//
// Возвращаемое значение:
//   Булево.
//
Функция ЕстьНеЦифры(Знач СтрокаПроверки) Экспорт
	...
	Иначе
		...
		Для Сч = 1 По СтрДлина(СтрокаПроверки) Цикл
			Если СписокКодовЧисел.НайтиПоЗначению(КодСимвола(Сред(СтрокаПроверки, Сч, 1))) = Неопределено Тогда
				Возврат Истина;
			КонецЕсли; 
		КонецЦикла; 
...
// Параметры:
//  Класс	 - Строка	 - класс
//  Объект	 - Метаданные	 - объект метаданных.
// 
// Возвращаемое значение:
//  Булево.
//
...
// 
// Возвращаемое значение:
//  ОписаниеТипов - с единственным типом набора записей.
//
...
// Служебная функция, представляющая десятеричное число в виде шестнадцатиричного.
//
// Параметры:
//  Источник - Число - Число
// 
// Возвращаемое значение:
//  Строка.
//
...
// Функция сравнивает два набора значений одного регистра.
//
// Параметры:
//  ТаблицаЗначений1 - ТаблицаЗначений	 - таблица 
//  ТаблицаЗначений2 - ТаблицаЗначений	 - таблица
// 
// Возвращаемое значение:
//  Булево.
//
...
//
// Параметры:
//  КоллекцияИсходная	 - Массив	 - Структура, Соответствие - исходная коллекция
// 
// Возвращаемое значение:
//  Массив, Структура, Соответствие - копия исходной коллекции
//
Функция СкопироватьУниверсальнуюКоллекцию(КоллекцияИсходная) Экспорт
	...
	Иначе
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Неверный тип универсальной коллекции для копирования");
		Возврат Неопределено
...
//
Процедура СвернутьТаблицуЗначений(Таблица, СуммируемыеКолонки = "")  экспорт
	
	КолонкиСуммы = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивСлов(СуммируемыеКолонки);
	
	ИзмеренияСтрокой = "";
	Для Каждого Колонка Из Таблица.Колонки Цикл
		Если КолонкиСуммы.Найти(Колонка.Имя) = Неопределено Тогда
			ОбщегоНазначенияКлиентСервер.КонкатенацияСтрок(ИзмеренияСтрокой, Колонка.Имя);
		КонецЕсли;
	КонецЦикла;
...
//
// Параметры:
//  СсылкаСправочника	 - СправочникСсылка	 - ссылка на справочник, объект которого проверяется;
...
//	Статус - СтатусСообщения - по умолчанию Неопределено
//
...
// Функция выводит в окно сообщений список сообщений
//
// Параметры:
//  СписокСообщений - Массив - список сообщений
Процедура СообщитьСписок(СписокСообщений) Экспорт
	
	#Если ВнешнееСоединение Тогда
	#Иначе
	Для Каждого ЭлементСообщения Из СписокСообщений Цикл
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ЭлементСообщения.Значение);	
	КонецЦикла;
 	#КонецЕсли

КонецПроцедуры

// Функция - Таблицу значений в табличный документ.
//
// Параметры:
//  Таблица		 - ТаблицаЗначений	 - Преобразуемая таблица значений.
//  ВывестиШапку - Булево	 - Вывести ли названия колонок таблицы в первую строку табличного документа.
// 
// Возвращаемое значение:
//   - ТабличныйДокумент.
//
Функция ТаблицуЗначенийВТабличныйДокумент(Таблица, ВывестиШапку = Ложь) Экспорт
	
	ТабДок = Новый ТабличныйДокумент;
	
	Если ВывестиШапку Тогда
		Для Сч = 1 По Таблица.Колонки.Количество() Цикл
			Колонка = Таблица.Колонки[Сч - 1];
			ТабДок.Область(1, Сч).Текст = ?(ЗначениеЗаполнено(Колонка.Заголовок), Колонка.Заголовок, Колонка.Имя);
		КонецЦикла;
	КонецЕсли;
	
	Для СчСтр = 1 По Таблица.Количество() Цикл
		Для Сч = 1 По Таблица.Колонки.Количество() Цикл
			
			ТабДок.Область(СчСтр + ?(ВывестиШапку, 1, 0), Сч).Текст = Таблица[СчСтр - 1][Сч - 1];
...
//
// Параметры:
//  СтрокаТабличнойЧасти - СтрокаТабличнойЧасти - ссылка на строку табличной части.
//
// Возвращаемое значение:
...
// 
// Возвращаемое значение:
//  Дата, МоментВремени - дата остатков.
//
...
//
// Параметры:
//  ЭтотОбъектМетаданные - ДокументОбъект	 - Сам объект или его метаданные
//  ИмяРеквизита		 - Строка			 - с именем реквизита
//  ИмяТабличнойЧасти	 - Строка			 - с именем табличной части
// 
// Возвращаемое значение:
//  Строка - Синоним реквизита объекта.
//
Функция ПолучитьСинонимРеквизита(ЭтотОбъектМетаданные,ИмяРеквизита="",ИмяТабличнойЧасти="") Экспорт
...
КонецФункции // ПолучитьСинонимРеквизита()

// Формирует структуру, содержащую значения реквизитов шапки документа.
//
// Параметры: 
//  ДокументОбъект - ДокументОбъект - документа для формирования структуры шапки, 
//
// Возвращаемое значение:
//  Структура.
//
Функция СформироватьСтруктуруШапкиДокумента(ДокументОбъект) Экспорт
	...
	ИмяДокумента = ДокументОбъект.Ссылка.Метаданные().Имя;
	Для Каждого Реквизит Из Метаданные.Документы[ИмяДокумента].Реквизиты Цикл
		Если ТипЗнч(ДокументОбъект) <> Тип("ДанныеФормыСтруктура")
			Или ДокументОбъект.Свойство(Реквизит.Имя)
		Тогда
...
//
// Параметры:
//  НаборДвижений				 - РегистрСведенийНаборЗаписей	 - набор движений регистра
//  ВидДвижения					 - ВидДвиженияНакопления - вид движения
//  ПустыеКолонкиСоставногоТипа	 - Структура			 - структура, содержащая имена измерений,ресурсов и
//  			реквизитов составного типа, которые могут содержать пустые ссылки
//  ЗаполнитьПериод				 - Булево				 - По умолчанию Истина.
//
...
//
// Параметры:
//  НаборДвижений			 - РегистрНакопленияНаборЗаписей	 - набор записей регистра движений.
//  ТабДвижений				 - ТаблицаЗначений	 - движения.
//  УдалятьПустыеДвижения	 - Булево		 - режим свертки.
//
Процедура СвернутьТаблицуДвиженийРегистраНакопления(НаборДвижений, ТабДвижений = Неопределено, УдалятьПустыеДвижения = Ложь) Экспорт
	...
	Если УдалятьПустыеДвижения Тогда
		...
		Для Сч = 0 По МассивПустСтрок.ВГраница() Цикл
			ТаблицаДвижений.Удалить(МассивПустСтрок[Сч]);
		КонецЦикла;
...
//
// Параметры:
//  Ссылка			 - ДокументСсылка			 - Ссылка на документ, для которого производится поиск движений по регистру
//  РегистрДвижения	 - РегистрНакопленияМенеджер - менеджер регистра, движение по которому пытаемся обнаружить
//  ПрочитатьЗаписи	 - Булево					 - Прочитать существующий набор записей для документа
//  					(если документ может формировать движения по указанному регистру)
//  Сообщать		 - Булево					 - выводить ли сообщения пользователю.
// 
// Возвращаемое значение:
...
// Следует использовать ОбщегоНазначенияКлиентСервер.ФормаСумм()
...
Функция ФорматСумм(Сумма, Валюта = Неопределено, ЧН = "", ЧРГ = "") Экспорт
	...
	Возврат ОбщегоНазначенияКлиентСервер.ФорматСумм(Сумма, Валюта, ЧН, ЧРГ);
...
//
// Возвращаемое значение:
//  Cтрока - представление периода для вывода в отчетах
//
...
//  МассивПрефиксов	 - Массив			 - префиксы.
// 
// Возвращаемое значение:
...
//
// Параметры:
//  МассивДанные - Массив		 - Массив
//  Элемент		 - Произвольный	 - Значение
// 
// Возвращаемое значение:
//  Число.
//
...
//
// Параметры:
//  Источник - Произвольный - первая переменная
//  Приемник - Произвольный - вторая переменная
//
...
// Присваивает значение источника приемнику. Истпользуется в типовых модулях 1С
//
// Параметры:
//  Источник - Произвольный - первая переменная
//  Приемник - Произвольный - вторая переменная
//
Процедура УстановитьНовоеЗначение(Источник, Приемник) Экспорт
...
//
// Параметры:
//  ИсходнаяСтрока	 - Строка	 - Строка
//  СимволПоиска	 - Строка	 - Символ
// 
// Возвращаемое значение:
//  Строка.
//
...
//
// Параметры:
//  ИмяФайла - Cтрока - содержащая имя файла, неважно с именем каталога или без
// 
// Возвращаемое значение:
//  Cтрока - расширение файла
//
...
//
// Параметры:
//  ПутьКФайлу	 - Cтрока	 - содержащая имя файла, неважно с именем каталога или без
// 
// Возвращаемое значение:
//  Cтрока - расширение файла
//
...
//
// Параметры:
//  ИмяФайла - Cтрока - содержащая полный путь к файла на диске
//
// Возвращаемое значение:
...
//
// Параметры:
//  ИмяКаталога	 - Cтрока	 - содержащая путь к каталогу файла на диске
//  ИмяФайла	 - Cтрока	 - содержащая имя файла, без имени каталога
// 
// Возвращаемое значение:
...
// 
// Возвращаемое значение:
//  Cтрока - каталог файлов для указанного объекта и пользователя.
//
...
//
// Параметры:
//  МетаданныеДокумента	 - Метаданные	 - ОбъектМетаданных: Документ.
//  ИмяТабличнойЧасти	 - Строка		 - Строка
//  СтрокаТабличнойЧасти - Строка		 - Строка
//  ИмяРеквизита		 - Строка		 - Строка
//  ПустоеЗначение		 - Произвольный	 - По умолчанию Неопределено
// 
// Возвращаемое значение:
//  Произвольный.
//
Функция ПолучитьЗначениеРеквизитаТЧ(МетаданныеДокумента, ИмяТабличнойЧасти, СтрокаТабличнойЧасти, ИмяРеквизита, ПустоеЗначение = Неопределено) Экспорт
	...
	ЗначениеРеквизита = ?(ОбщегоНазначения.ЕстьРеквизитТабЧастиДокумента(ИмяРеквизита, МетаданныеДокумента, ИмяТабличнойЧасти),
...
////////////////////////////////////////////////////////////////////////////////
// ПРОЦЕДУРЫ И ФУНКЦИИ РАБОТЫ С ФИО
...
//
// Параметры:
//  Объект	 - Строка	 - строка или ссылка или объект элемента спр-ка ФизическиеЛица.
//  Фамилия	 - Строка	 - фамилия физ. лица.
//  Имя		 - Строка	 - имя физ. лица.
//  Отчество - Строка	 - отчество физ. лица.
// 
// Возвращаемое значение:
//  Строка - Фамилия и Инициалы одной строкой. Побочные эффекты - переданная целая строка.
//	  Побочные эффекты - переданная целая строка разбивается на подстроки, соответствующие
//	  отдельным Фамилии,Имени и Отчеству.
//
Функция ФамилияИнициалыФизЛица(Объект = Неопределено, Знач Фамилия = " ", Знач Имя = " ", Знач Отчество = " ") Экспорт

	ТипОбъекта = ТипЗнч(Объект);
	Если ТипОбъекта = Тип("Строка") Тогда
		ФИО = ОбщегоНазначения.РазложитьСтрокуВМассивПодстрок(СокрЛП(Объект)," ");
	ИначеЕсли ТипОбъекта = Тип("СправочникСсылка.Клиенты")	  	Или	ТипОбъекта = Тип("СправочникОбъект.Клиенты") Или 
			  ТипОбъекта = Тип("СправочникСсылка.Сотрудники")	Или	ТипОбъекта = Тип("СправочникОбъект.Сотрудники") 
	Тогда 
		Если ЗначениеЗаполнено(Объект.Фамилия) И ЗначениеЗаполнено(Объект.Имя) И ЗначениеЗаполнено(Объект.Отчество) Тогда
			ФИО = ОбщегоНазначения.РазложитьСтрокуВМассивПодстрок(СокрЛП(Объект.Фамилия + " " + Объект.Имя + " " + Объект.Отчество)," ");
	    Иначе
...
//
// Параметры:
//  Объект - СправочникСсылка, Строка - строка или ссылка или объект элемента спр-ка ФизическиеЛица.
//
// Возвращаемое значение: 
//  Строка - Фамилия и Инициалы одной строкой. Побочные эффекты - переданная целая строка.
//  		Побочные эффекты - переданная целая строка разбивается на подстроки, соответствующие
//  		отдельным Фамилии,Имени и Отчеству.
//
Функция ФИОФизЛица(Объект) Экспорт

	ТипОбъекта = ТипЗнч(Объект);
	Если  ТипОбъекта = Тип("СправочникСсылка.Клиенты")	  	Или	ТипОбъекта = Тип("СправочникОбъект.Клиенты") Или 
		  ТипОбъекта = Тип("СправочникСсылка.Сотрудники")	Или	ТипОбъекта = Тип("СправочникОбъект.Сотрудники") 
	Тогда
		ФИО = СокрЛП(СокрЛП(Объект.Фамилия) + " " + СокрЛП(Объект.Имя) + " " + СокрЛП(Объект.Отчество));
		ФИО = СтрЗаменить(ФИО, "  ", " ");
		Возврат ФИО;
	Иначе                                                           
...
////////////////////////////////////////////////////////////////////////////////
// ПРОЦЕДУРЫ И ФУНКЦИИ РАБОТЫ С КЛИЕНТАМИ
                                                                                
// Функция определяет пол человека по его ФИО
//
// Параметры:
//  Имя		 - Строка	 - Имя, по последней букве которого пытаемся определить пол, По умолчанию ""
//  Фамилия	 - Строка	 - Фамилия, по окончанию которой уточняется пол, По умолчанию ""
//  Отчество - Строка	 - Отчество, по окончанию которого пытаемся определить пол, По умолчанию ""
// 
// Возвращаемое значение:
//  ПеречислениеСсылка.ПолФизическихЛиц.
//
Функция ОпределитьПол(Имя = "", Фамилия = "", Отчество = "") Экспорт
...
// Обработчик подписки на событие ПриУстановкеНовогоНомераДокумента.
//
// Параметры:
//  Источник			 - ДокументОбъект	 - источник.
//  СтандартнаяОбработка - Булево			 - продолжение обработки.
//  Префикс				 - Строка			 - префикс кода.
//
Процедура ПриУстановкеНовогоНомераДокументаПриУстановкеНовогоНомера(Источник, СтандартнаяОбработка, Префикс) Экспорт
	ОбщегоНазначения.ДобавитьПрефиксКассы(Источник, Префикс);
	ОбщегоНазначения.ДобавитьПрефиксУзла(Префикс);
КонецПроцедуры

// Обработчик подписки на событие ПриУстановкеНовогоКодаСправочника.
//
// Параметры:
//  Источник			 - СправочникОбъект	 - источник.
//  СтандартнаяОбработка - Булево			 - продолжение обработки.
//  Префикс				 - Строка			 - префикс кода.
//
...
// Функция преобразует значение время значения типа Дата в Число - число секунд этого времени.
//
// Параметры:
//  ДатаВремя	 - Дата	 - дата.
// 
...
// Функция возращает истина если Элемент имеет предка Предок в иерархичном справочнике.
//
// Параметры:
//  Элемент	 - СправочникСсылка	 - проверяемый элемент справочника.
//  Предок	 - СправочникСсылка	 - возможный элемент-предок справочника.
// 
// Возвращаемое значение:
//  Булево.
//
Функция ПроверкаРодственнойСвязи(Элемент,Предок) Экспорт
	Если Элемент.Родитель.Ссылка.Пустая() Тогда Возврат Ложь; КонецЕсли;
	Если Предок = Элемент.Родитель Тогда Возврат Истина; КонецЕсли;
	Если Элемент.Родитель.Родитель.Ссылка.Пустая() Тогда
		Возврат Ложь;	
	Иначе	
		Если ПроверкаРодственнойСвязи(Элемент.Родитель,Предок) Тогда Возврат Истина КонецЕсли;
    КонецЕсли;
    Возврат Ложь;
КонецФункции

// Функция возвращает строковое представление дня недели заданной даты.
//
// Параметры:
//  Дата - Дата	 - дата
// 
...
Функция ПолучитьНаименованиеДняНедели(Дата) Экспорт 
	...
	мДеньНедели = ДеньНедели(Дата);
	СтруктураНаименований = Новый Структура("Понедельник,Вторник,Среда,Четверг,Пятница,Суббота,Воскресенье"
	                                         ,1,2,3,4,5,6,7);
	Для Каждого КлючЗначение Из СтруктураНаименований Цикл
		Если КлючЗначение.Значение = мДеньНедели Тогда
...
// Добавляет к префиксу номера документа префикс кассы, выбранной в документе.
//
// Параметры:
//  ДокументОбъект	 - ДокументОбъект	 - документ.
//  Префикс			 - Строка			 - префикс.
//
Процедура ДобавитьПрефиксКассы(ДокументОбъект, Префикс) Экспорт

	ИмяДокумента = ДокументОбъект.Метаданные().Имя;
	Если НЕ (ИмяДокумента="ПриходныйКассовыйОрдер" ИЛИ ИмяДокумента="РасходныйКассовыйОрдер")
		 ИЛИ НЕ ЗначениеЗаполнено(ДокументОбъект.Касса.Префикс)
	Тогда
...
//
Процедура ПоместитьТзВоВременнуюТаблицу(ТаблицаЗначений, ИмяВременнойТаблицы, Запрос, СписокПолей = "") Экспорт
...
// Заполняет в коллекции свойство "ВидНоменклатуры" по свойству "Номенклатура".
// Параметры:
//  КоллекцияНоменклатуры - Массив.
//
Процедура ЗаполнитьВидНоменклатурыВКоллекции(КоллекцияНоменклатуры) Экспорт
	
	Если ТипЗнч(КоллекцияНоменклатуры) = Тип("ТаблицаЗначений")
		И КоллекцияНоменклатуры.Колонки.Найти("ВидНоменклатуры") = Неопределено
	Тогда
		КоллекцияНоменклатуры.Колонки.Добавить("ВидНоменклатуры", Новый ОписаниеТипов("ПеречислениеСсылка.ВидыНоменклатуры"));
	КонецЕсли;
	
	МассивНоменклатуры = Новый Массив;
	Для Каждого ЭлементКоллекции Из КоллекцияНоменклатуры Цикл
		МассивНоменклатуры.Добавить(ЭлементКоллекции.Номенклатура);
	КонецЦикла;
	...
	Для Каждого ЭлементКоллекции Из КоллекцияНоменклатуры Цикл
		...
		Отбор.Номенклатура = ЭлементКоллекции.Номенклатура;
		Выб.Сбросить();
		Если Выб.НайтиСледующий(Отбор) Тогда
			ВидНоменклатуры = Выб.ВидНоменклатуры;
		Иначе
			ВидНоменклатуры = Перечисления.ВидыНоменклатуры.ПустаяСсылка();
		КонецЕсли;
		
		Если ТипЗнч(КоллекцияНоменклатуры) = Тип("ТаблицаЗначений") Тогда
			ЭлементКоллекции.ВидНоменклатуры = ВидНоменклатуры;
		ИначеЕсли ТипЗнч(КоллекцияНоменклатуры) = Тип("Массив") Тогда
			ЭлементКоллекции.Вставить("ВидНоменклатуры", ВидНоменклатуры);
		КонецЕсли;
...
// Формирует соотвествие, хранящее виды номенклатуры для ссылок на номенклатуру.
//
// Параметры:
//  КоллекцияНоменклатуры	 - ТаблицаЗначений - Коллекция с номенклатурой, соответсвие которой нужно сформировать.
//
// Возвращаемое значение
// 	Соответствие - ключ - Номенклатура, значение - Вид номенклатуры.
//
// Возвращаемое значение:
//  Неопределено.
//
Функция ПолучитьСоответствиеВидовНоменклатуры(Знач КоллекцияНоменклатуры) Экспорт
	...
	Если КоллекцияНоменклатуры.Количество() <> 0 Тогда
		...
		Если ТипЗнч(КоллекцияНоменклатуры) = Тип("ТаблицаЗначений") Тогда
			МассивНоменклатуры = КоллекцияНоменклатуры.Выгрузить().ВыгрузитьКолонку("Номенклатура");
		ИначеЕсли ТипЗнч(КоллекцияНоменклатуры) = Тип("Массив") Тогда
			МассивНоменклатуры = КоллекцияНоменклатуры;
		Иначе
		...
		Пока Выб.Следующий() Цикл
			ВидыНоменклатуры.Вставить(Выб.Номенклатура, Выб.ВидНоменклатуры);
		КонецЦикла;
	КонецЕсли;
...
//
// Параметры:
//  МасСотр - Неопределено
//  ИмяРеквизита - Строка - По умолчанию "Сотрудник"
//
// Возвращаемое значение:
// 	Cтрока - значение реквизита "КраткийСоставДокумента".
//
Функция ЗаполнитьКраткийСоставДокумента(МасСотр, ИмяРеквизита = "Сотрудник") Экспорт
	...
	Если НЕ КраткийСоставДокументаЗаполнен Тогда
		...
		Для Инд = 0 По МасСотр.Количество() - 1 Цикл
			
			ФИОФизЛица = ОбщегоНазначения.ФамилияИнициалыФизЛица(МасСотр[Инд]);
...
//
Функция ПолучитьКраткоеНаименованиеОсновнойВалюты() Экспорт
...
// Функция возвращает признак того, что текущий сеанс программы является сейчас единственным сеансом
// программы, работающим в режиме "Предприятие".
// Возвращаемое значение:
//  Неопределено.
//
...
// Функция возвращает признак того, что база работает в режиме файл-сервер.
// Параметры:
//  СтрокаСоединенияСБД - Строка - По умолчанию ""
//
// Возвращаемое значение:
//  Булево.
//
...
// Функция возвращает признак того, что текущий узел РИБ имеет главный узел.
// Возвращаемое значение:
//  Булево.
//
Функция ЭтоПодчиненныйУзел() Экспорт
	   
	Возврат ПараметрыСеанса.ИспользованиеРИБ И ПланыОбмена.ГлавныйУзел() <> Неопределено;
...
//
// Параметры:
//  Организация	 - СправочникСсылка	 - для которой нужно получить префиксы.
// 
// Возвращаемое значение:
//  Массив - префиксы.
//
...
// Возвращает массив всех родителей элемента иерархического справочника.
// Параметры:
...
//  Массив.
//
Функция ПолучитьСписокРодителейЭлемента(СсылкаНаОбъект) Экспорт
...
Функция ЕстьПравоДоступа(Право, Знач ОбъектМетаданных, Пользователь = Неопределено) Экспорт 
	...
	Возврат ПравоДоступа(Право, ОбъектМетаданных, Пользователь);
	
КонецФункции

// Заполняет область макета значением параметра посимвольно.
// Параметры:
//  ОбластьМакета - Неопределено
//  БазовоеИмяПараметра - Строка
//  СтрокаЗначение - Строка
//
Процедура ЗаполнитьПараметрПосимвольно(ОбластьМакета,БазовоеИмяПараметра, Знач СтрокаЗначение) Экспорт 
...
// Убирает из строки лишние повторения символа.
// Параметры:
//  СтрокаДо - Строка
//  Символ - Строка - По умолчанию ","
//
// Возвращаемое значение:
//  Строка.
//
Функция УбратьПовторяющиесяСимволы(Знач СтрокаДо, Символ = ",") Экспорт
	ПовтСимвол = Символ + Символ;
	СтрокаПосле = СтрЗаменить(СтрокаДо, ПовтСимвол, Символ);
	Пока СтрДлина(СтрокаДо) <> СтрДлина(СтрокаПосле) Цикл
		СтрокаДо = СтрокаПосле;
		СтрокаПосле = СтрЗаменить(СтрокаДо, ПовтСимвол, Символ); 
	КонецЦикла;
...
//       
//       * ВключатьБизнесЛогику - Булево - Необязательный. Режим записи мест использования при замене дублей на
//                                         оригиналы. Истина - По умолчанию. Места использования дублей записываются в
//                                         режиме ОбменДанными.Загрузка = Ложь. Ложь   - Запись ведется в режиме
//                                         ОбменДанными.Загрузка = Истина.
//       
//       * ЗаменаПарыВТранзакции - Булево - Необязательный. Определяет размер транзакции.
//           Истина - По умолчанию. Транзакция охватывает все места использования одного дубля. Может быть очень
//                    ресурсоемко  в случае большого количества мест использований.
//           Ложь   - Замена каждого места использования выполняется в отдельной транзакции.
//       
//       * ПривилегированнаяЗапись - Булево - Необязательный. Требуется ли устанавливать привилегированный режим перед
//                                            запись. Ложь   - По умолчанию. Записывать с текущими правами.
...
//           "НеизвестныеДанные" - при обработке были найдены данные, которые не планировались к анализу, замена не
//                                 реализована.
...
//
// Параметры:
//  ПереданныеПараметры             - Структура - настройки подключения (необходимо использовать функцию
...
//   * Местоположение               - Строка - (необязательный) фактический адрес сервиса. Используется, если адрес
//                                             реального расположения сервиса отличается от адреса, который указан в
//                                             WSDL-файле.
...
Функция СоздатьWSПрокси(ПереданныеПараметры) Экспорт
	...
	Если ДелатьКонтрольныйВызов И Таймаут <> Неопределено И Таймаут > 20 Тогда
		...
		WSПроксиPing = ВнутренняяWSПрокси(ПараметрыПодключения);
		Попытка
			WSПроксиPing.Ping();
		Исключение
...
// Процедура ОбработатьОбъектСПерехватомСообщений.
//
// Параметры:
//  Знач Объект - Неопределено
//  Знач Действие - Неопределено
//  Знач РежимЗаписи - Неопределено
//  Знач ПараметрыЗаписи - Неопределено
//
Процедура ОбработатьОбъектСПерехватомСообщений(Знач Объект, Знач Действие, Знач РежимЗаписи, Знач ПараметрыЗаписи) Экспорт 
...
// Бит+  перенос из регистра сведений КэшПрограммныхИнтерфейсов.
