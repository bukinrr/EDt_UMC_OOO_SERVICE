...
Функция ПолучитьРезультатЗапросаДляВыгрузкиОчисткиДанных(Свойства, ИмяТипа, 
	...
	ИначеЕсли ИмяТипа = "Документ" Тогда
		...
		//+бит
		ИтоговоеОграничениеПоФилиалам = ПолучитьСтрокуОграниченияПоФилиаламДляЗапроса(Свойства.Имя, ИмяТипа, "ОбъектДляВыгрузки", ИтоговоеОграничениеПоДате); 
		ИтоговоеОграничениеПоОрганизациям = ПолучитьСтрокуОграниченияПоОрганизациямДляЗапроса(Свойства.Имя, ИмяТипа, "ОбъектДляВыгрузки", ИтоговоеОграничениеПоДате); 	
		//-бит
		...
		//+бит
		Запрос.УстановитьПараметр("Филиалы", бит_ФилиалыВыгрузки);
		Запрос.УстановитьПараметр("Организации", бит_ОрганизацииВыгрузки);	
		// -бит
		...
		Запрос.Текст = "ВЫБРАТЬ " + СтрокаРазрешения + "
				 ...
				 //+бит
				 + " " + ИтоговоеОграничениеПоФилиалам
				 + " " + ИтоговоеОграничениеПоОрганизациям;
				 //-бит				
...
//+бит
Функция ПолучитьСтрокуОграниченияПоФилиаламДляЗапроса(ИмяМетаданныхДокумента, ИмяТипа, ИмяГруппыТаблицы = "", ИтоговоеОграничениеПоДате) Экспорт
	
	ИтоговоеОграничениеПоФилиалам = "";
	
	Если НЕ (ИмяТипа = "Документ") Тогда
		Возврат ИтоговоеОграничениеПоФилиалам;
	КонецЕсли;
	
	МетаданныеДокумента = Метаданные.НайтиПоПолномуИмени("Документ." + ИмяМетаданныхДокумента);
	Если МетаданныеДокумента = Неопределено Тогда 
		Возврат ИтоговоеОграничениеПоФилиалам;
	КонецЕсли;
	
	ИмяФилиал = ПроцедурыСпециализацииПоставки.ИмяРеквизитаФилиалДокументов();
	Если Не ОбщегоНазначения.ЕстьРеквизитДокумента(ИмяФилиал, МетаданныеДокумента) Тогда 
		Возврат ИтоговоеОграничениеПоФилиалам;
	КонецЕсли;
	
	ИмяПоляОграничения = ?(ПустаяСтрока(ИмяГруппыТаблицы), ИмяФилиал, ИмяГруппыТаблицы + "." + ИмяФилиал);
	
	Если ЗначениеЗаполнено(бит_ФилиалыВыгрузки) Тогда
		
		Если ИтоговоеОграничениеПоДате = "" Тогда  		
			ИтоговоеОграничениеПоФилиалам = "
			|	ГДЕ
			|		" + ИмяПоляОграничения + " В (&Филиалы)";
		Иначе		
			ИтоговоеОграничениеПоФилиалам = "
			|	И
			|		" + ИмяПоляОграничения + " В (&Филиалы)";
...
Функция ПолучитьСтрокуОграниченияПоОрганизациямДляЗапроса(ИмяМетаданныхДокумента, ИмяТипа, ИмяГруппыТаблицы = "", ИтоговоеОграничениеПоДате) Экспорт
	
	ИтоговоеОграничениеПоОрганизациям = "";
	
	Если НЕ (ИмяТипа = "Документ") Тогда
		Возврат ИтоговоеОграничениеПоОрганизациям;
	КонецЕсли;
	...
	ИмяПоляОграничения = ?(ПустаяСтрока(ИмяГруппыТаблицы), ИмяФилиал, ИмяГруппыТаблицы + "." + ИмяФилиал + "." + "Организация");
	
	Если ЗначениеЗаполнено(бит_ОрганизацииВыгрузки) Тогда
		
		Если ИтоговоеОграничениеПоДате = "" Тогда  		
			ИтоговоеОграничениеПоОрганизациям = "
			|	ГДЕ
			|		" + ИмяПоляОграничения + " В (&Организации)";
		Иначе		
			ИтоговоеОграничениеПоОрганизациям = "
			|	И
			|		" + ИмяПоляОграничения + " В (&Организации)";
