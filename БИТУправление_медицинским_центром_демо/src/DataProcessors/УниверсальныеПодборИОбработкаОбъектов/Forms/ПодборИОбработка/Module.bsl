	...
	Иначе
		...
		Если ОтборДанных <> Неопределено Тогда
			Для Каждого ЭлементОтбора Из ОтборДанных.Отбор.Элементы Цикл
				...
				|	И ") + ОбработкаОбъект.ПолучитьВидСравнения(ЭлементОтбора.ЛевоеЗначение, ЭлементОтбора.ВидСравнения, ИмяПараметра);
...
Процедура ВыполнитьОбработку(Команда)
	...
	Для каждого Строка из ВыбранныеОбработки Цикл
		...
		Если НЕ ОбработкаДоступна(ОбъектПоиска.Тип, ИмяФормыОбработки) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Обработка " + ИмяФормы + " недоступна для типа <" + ОбъектПоиска.Тип + ">");
			Продолжить;
		КонецЕсли;
...
Функция ПолучитьПолноеИмяФормы(ИмяНужнойФормы)
	...
	МассивСтрок = ОбработкаОбъект.РазложитьСтрокуВМассивПодстрок(ЭтаФорма.ИмяФормы, ".");
	МассивСтрок[МассивСтрок.Количество() - 1] = ИмяНужнойФормы;
	
	Возврат ОбработкаОбъект.ПолучитьСтрокуИзМассиваПодстрок(МассивСтрок, ".");
КонецФункции
...
Процедура ДоступныеОбработкиВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(ОбъектПоиска) Тогда
		ПоказатьПредупреждение(,"Не выбран ""Объект поиска""");
		Возврат;
	КонецЕсли;
...
Процедура ВыбранныеОбработкиПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Поле)
	...
	ЭлементыДоступныхОбр = ДоступныеОбработки.ПолучитьЭлементы();
	Для Каждого СтрВыбранных Из ПараметрыПеретаскивания.Значение Цикл
		
		Если СтрВыбранных.ПолучитьРодителя() = Неопределено Тогда
			Продолжить;
		КонецЕсли;
	    СтрДоступных = СтрВыбранных;
		
		НовСтр = ВыбранныеОбработки.Добавить();
		НовСтр.ОбработкаНастройка = СтрДоступных.Обработка;
		НовСтр.СтрокаДоступнойОбработки = СтрВыбранных.ПолучитьИдентификатор();
		НовСтр.Выбрать = Истина;
