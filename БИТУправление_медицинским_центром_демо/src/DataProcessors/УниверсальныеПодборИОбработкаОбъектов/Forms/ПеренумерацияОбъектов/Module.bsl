...
&НаКлиенте
Перем мИспользоватьНастройки Экспорт; // Признак использования настроек

// Типы объектов, для которых может использоваться обработка.
// По умолчанию для всех.
&НаКлиенте
Перем мТипыОбрабатываемыхОбъектов Экспорт; // Типы объектов, для которых может использоваться обработка.
...
Процедура ПроверитьНеУникальныеНомера(НеУникальныеНомера)
	Для каждого Зн Из НеУникальныеНомера Цикл
		...
		Попытка
			Объект.Записать();
		Исключение
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Повтор номера: " + НовыйНомер + " за пределами данной выборки!");
		КонецПопытки;		
	КонецЦикла;
КонецПроцедуры
...
Функция ВыполнитьОбработку() Экспорт
	...
	// бит.Куликов Сергей.19.06.2018
	ОбновитьСчетчикНумерации(Тип(ОбъектПоиска.Тип + "Ссылка." + ОбъектПоиска.Имя));
	// бит.Куликов Сергей.19.06.2018
...
Процедура ОбновитьСчетчикНумерации(ТипДанных)
	
	ОбновитьНумерациюОбъектов(Метаданные.НайтиПоТипу(ТипДанных));
