...
#Область РазделОписанияЭкспортныхПеременных
...
#Область ВспомогательныеПеременныеМодуляДляНаписанияАлгоритмов
...
#Область РазделОписанияПеременныхОбработокОбмена
...
#Область РазделОписанияПеременныхМодуляОбработкиВыгрузки
...
#Область РазделОписанияПеременныхМодуляОбработкиЗагрузки
...
#Область РазделОписанияПеременныхДляЗначенийСвойств
...
Перем ПолеВыгружаемыеПоСсылкеОбъектыМетаданных; // (кэш) Соответствие: Ключ - ОбъектМетаданных; Значение - признак
                                                // выгрузки объекта по ссылке: Истина - надо выгружать объект по
                                                // ссылке, Ложь - не надо.

Перем ПолеПравилаРегистрацииОбъектов; // (кэш) ТаблицаЗначений - содержит правила регистрации объектов (правила только
                                      // с видом "ФРО" и для текущего плана обмена).
...
// 
// Возвращаемое значение:
//  ПеречислениеСсылка.РезультатыВыполненияОбмена.
//
...
//
// Параметры:
//	ОбработкаДляЗагрузкиДанных - Неопределено - обработка-объект.
//
...
// Процедура ВнешнееСоединениеВыполнитьЗагрузкуДанныхИзСтрокиXML.
//
// Параметры:
//  СтрокаXML - Строка
//
...
// Процедура ВнешнееСоединениеОбработчикКонвертацииПередЗагрузкойДанных.
//
// Параметры:
//  Отказ - Булево
//
...
// Процедура ВнешнееСоединениеПередЗагрузкойДанных.
//
...
// Процедура ВнешнееСоединениеПослеЗагрузкиДанных.
//
...
// Процедура ВнешнееСоединениеПроверитьНачалоИФиксациюТранзакцииПриЗагрузкеДанных.
//
...
// Процедура ВнешнееСоединениеНачатьТранзакциюПриЗагрузкеДанных.
//
...
// Процедура ВнешнееСоединениеЗафиксироватьТранзакциюПриЗагрузкеДанных.
//
...
//  Длина        - Строка - требуемая длина строки
//	ДобавлятьНулиЕслиДлинаНеМеньшеТекущейДлиныНомера - Булево - по умолчанию Истина
...
//  Длина        - Строка - требуемая результирующая длина строки
//  Режим        - Строка - "Слева" - подстрока добавляется слева к префиксу, иначе - справа
//	НеДобавлятьПрефиксЕслиСНегоНачинаетсяНомер - Булево
//	ОбрезатьЧисловуюЧастьСлеваВСлучаеПревышенияНомера - Булево
//
// Возвращаемое значение:
...
//
// Параметры:
//  Значение - ПеречислениеСсылка	 - значение произвольного перечисления.
// 
// Возвращаемое значение:
...
//
// Параметры: 
//  Значение - Произвольный - значение, заполнение которого надо проверить.
//
// Возвращаемое значение:
...
//
// Параметры: 
//  УзелРодитель   - УзелXML - xml узел родитель.
//  Имя            - Строка - имя добавляемого узла.
//
// Возвращаемое значение:
//  ЗаписьXML - Новый xml-узел, добавленный к указанному узлу-родителю.
//
...
//
// Параметры: 
//  Узел           - ЗаписьXML - копируемый узел.
//
// Возвращаемое значение:
//  ЗаписьXML - Новый xml - копия указанного узла.
//
...
//  Узел		 - ЗаписьXML - подчиняемый узел .
//
...
//  Тип		 - Тип		 - Значение типа Тип. Тип атрибута.
//  Имя		 - Строка	 - Имя атрибута.
// 
// Возвращаемое значение:
...
//  ИскатьПоСвойству	 - Булево	 - для ссылочных типов может быть указано свойство, по которому
//  	следует искать объект: "Код", "Наименование", <ИмяРеквизита>, "Имя" (предопределенного значения).
//  ОбрезатьСтрокуСправа - Булево - сокращать ли строку.
// 
// Возвращаемое значение:
...
Функция ПрочитатьСвойство(Тип, НеСоздаватьОбъектЕслиНеНайден = Ложь, СвойствоНеНайденоПоСсылке = Ложь, ИмяПКО = "")
	...
	Пока ФайлОбмена.Прочитать() Цикл
		...
		ИначеЕсли ИмяУзла = "Ссылка" Тогда
			...
			Если ПродолжитьПоискПоПолямПоискаЕслиПоИдентификаторуНеНашли
				// Необходимо добавить запись в регистр сопоставления объектов.
				И СоответствияОбъектовИнформационныхБаз <> Неопределено Тогда
				
				Объект = ПолучитьОбъектПоСсылкеИДопИнформации(СозданныйОбъект, Значение);
...
Процедура ЗагрузитьТабличнуюЧасть(Объект, ИмяТабличнойЧасти, ОбщаяИнформацияОТипеДокумента, ПараметрыОбъекта, ПКО)
	...
	// +бит.Куликов Сергей.12.05.2014
	Попытка
		МетаданныеОбъекта = Объект.Метаданные();
		Если Объект.ЭтоГруппа
			И Строка(МетаданныеОбъекта.ТабличныеЧасти.Найти(ИмяТабличнойЧасти).Использование) = "ДляЭлемента"
		Тогда
			одПропустить(ФайлОбмена);
			Возврат;
		КонецЕсли;
	...
	Попытка
		фМетаданные = Объект.Метаданные();
		Если фМетаданные.ТабличныеЧасти.Найти(ИмяТабличнойЧасти) <> Неопределено Тогда  
...
Функция ПроверитьНаличиеКоллизийДляОбъекта(Объект, ИмяТипаОбъекта, ДополнительнаяИнформация = "", ПриоритетОбъектаОбмена)
	...
	Если ЕстьКоллизия Тогда
		...
		ЗП = Новый Структура;
		Если Найти(Строка(ТипЗнч(Объект)), "набор записей") > 0 Тогда
		Иначе
...
// Процедура ВыгрузитьРегистр.
//
// Параметры:
//  НаборЗаписейДляВыгрузки - Неопределено
//  Правило - Неопределено - По умолчанию Неопределено
//  ВходящиеДанные - Неопределено - По умолчанию Неопределено
//  НеВыгружатьОбъектыПоСсылкам - Булево - По умолчанию Ложь
//  ИмяПКО - Строка - По умолчанию ""
//  ПравилоВыгрузкиДанных - Неопределено - По умолчанию Неопределено
//
...
// Функция ВыгрузкаРегистра.
//
// Параметры:
//  НаборЗаписейДляВыгрузки - Неопределено
//  Правило - Неопределено - По умолчанию Неопределено
//  ВходящиеДанные - Неопределено - По умолчанию Неопределено
//  НеВыгружатьОбъектыСвойствПоСсылкам - Булево - По умолчанию Ложь
//  ИмяПКО - Строка - По умолчанию ""
//
// Возвращаемое значение:
//  Неопределено.
...
// Функция ПолучитьРезультатЗапросаДляВыгрузкиОчисткиДанных.
//
// Параметры:
//  Свойства - Неопределено
//  ИмяТипа - Строка
//  ВыборкаДляОчисткиДанных - Булево - По умолчанию Ложь
//  УдалятьОбъектыНепосредственно - Булево - По умолчанию Ложь
//  ВыбиратьВсеПоля - Булево - По умолчанию Истина
//
// Возвращаемое значение:
//  Неопределено.
//
...
// Функция ПолучитьВыборкуДляВыгрузкиОчисткиДанных.
//
// Параметры:
//  Свойства - Неопределено
//  ИмяТипа - Строка
...
//
// Параметры:
//  Правило        - Неопределено - ссылка на правило выгрузки данных.
//
...
//	СтрокаСообщенияОбОшибке - Строка - по умолчанию ""
//	ЗагружатьТолькоЗаголовокПравил - Булево - по умолчанию Ложь
//
...
Процедура ВыполнитьОтложенноеПроведениеДокументов()
	...
	Для Каждого СтрокаТаблицы Из ДокументыДляОтложенногоПроведения() Цикл
		...
		// +Бит К функционалу БСП добавляем свой признак проведения при загрузке (стандартный при проведении недопустимый).
		Объект.ДополнительныеСвойства.Вставить("ОбменДаннымиЗагрузка");
		// -бит
		...
		// К функционалу БСП добавляем свой признак проведения при загрузке (удаление) (// +бит).
		Объект.ДополнительныеСвойства.Удалить("ОбменДаннымиЗагрузка");
		// End(-бит)
...
Процедура ВыполнитьВыгрузкуЗарегистрированныхДанных(ЗаписьСообщения, СтрокаСообщенияОбОшибке, ТаблицаПравилВыгрузкиИспользуемые)
	...
	Пока ВыборкаИзменений.Следующий() Цикл
		...
		Если ТипДанныхДляВыгрузки <> ТипРегистрСоответствия Тогда
			...
			ОбменДаннымиСобытия.ПриОтправкеДанныхОбменСФилиалами(Данные, ОтправкаЭлемента, УзелДляОбмена, ИмяПланаОбмена(), НачальнаяВыгрузкаДанных);
...
Функция ВерсияФорматаВходящегоСообщенияОбмена()
	...
	Если РазрядыВерсий.Количество() < 4 Тогда
		...
		Для счетчик = 1 По КоличествоРазрядовДобавить Цикл
