...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЕстьМестоДеятельностиПолучатель = ОбщегоНазначения.ЕстьРеквизитОбъекта("МестоДеятельностиПолучатель", МетаданныеОбъекта);
	...
	Если СтруктураБыстрогоОтбора <> Неопределено Тогда
		...
		Если СтруктураБыстрогоОтбора.Свойство("МестоДеятельности", МестоДеятельности) Тогда
			...
			ИначеЕсли ЕстьМестоДеятельности Тогда
				ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
	...
	Элементы.МестоДеятельностиОтбор.Видимость = ЕстьМестоДеятельности Или ЕстьМестоДеятельностиПолучатель;
	Элементы.МестоДеятельности.Видимость      = ЕстьМестоДеятельности Или ЕстьМестоДеятельностиПолучатель;
...
Процедура ЗаполнитьСписокВыбораСтатуса(МенеджерОбъекта)
	...
	Если ЕстьИсходящие Тогда
		...
		СписокСтатусов.Добавить(Перечисления.СтатусыИнформированияМДЛП.ЧастичноПередано);
		СписокСтатусов.Добавить(Перечисления.СтатусыИнформированияМДЛП.Отклонено);
		СписокСтатусов.Добавить(Перечисления.СтатусыИнформированияМДЛП.ПринятоСОтклонениемОтТребований);
	КонецЕсли;
	
	Если ЕстьВходящие Тогда
		СписокСтатусов.Добавить(Перечисления.СтатусыИнформированияМДЛП.ПринятоИзМДЛП);
		СписокСтатусов.Добавить(Перечисления.СтатусыИнформированияМДЛП.ОприходованоНовымВладельцем);
		Если ОбщегоНазначения.ПодсистемаСуществует("ИнтеграцияМДЛП.ИмпортЭкспорт.Импорт") Тогда
			СписокСтатусов.Добавить(Перечисления.СтатусыИнформированияМДЛП.ВвезеноВРФБезПодтверждения);
		КонецЕсли;
	КонецЕсли;
...
Процедура ВыполнитьОбмен(Команда)
	...
	ПараметрыВыполненияОбмена.ТипыДокументов = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ТипЗнч(Параметры.Уведомление));
	
	ИнтеграцияМДЛПКлиент.ВыполнитьОбмен(Организация, МестоДеятельности, ПараметрыВыполненияОбмена);
