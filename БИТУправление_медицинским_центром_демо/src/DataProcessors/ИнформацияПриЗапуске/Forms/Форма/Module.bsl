...
Процедура Подключаемый_ПерейтиКСтранице(Команда)
	...
	Результат = ПросмотрСтраницы("КомандаИзКоманднойПанели", Команда.Имя);
	
	Если ТипЗнч(Результат) = Тип("Структура") Тогда
		
		ИмяВременногоФайла = ПолучитьИмяВременногоФайла(Результат.Расширение);
		ДвоичныеДанные = ПолучитьИЗВременногоХранилища(Результат.Адрес);
...
Функция ПросмотрСтраницы(ТипДействия, Параметр = Неопределено)
	...
	Для Каждого стрКоманды Из СтрокаКоманды.ВебСтраницы Цикл
		Если стрКоманды.Расширение = "xls" Тогда
			
			ДанныеФайла = Новый Структура("Адрес, Расширение");
			ЗаполнитьЗначенияСвойств(ДанныеФайла, стрКоманды);
			
			Возврат ДанныеФайла;
	
		ИначеЕсли ЗначениеЗаполнено(АдресСтраницы) Тогда 
...
Процедура РазместитьСтраницыКоманды(СтрокаТаблицы)
	...
	Для Каждого Файл Из Найденные Цикл
		...
		Если Расширение = "htm" ИЛИ Расширение = "html" ИЛИ Расширение = "xls" Тогда
			...
			РазмещениеФайла.Расширение = Расширение;
		Иначе
	...
	Для НомерСтроки = 1 По СтрокаТаблицы.ВебСтраницы.Количество() Цикл
		...
		Если Расширение = "xls" Тогда 
			РазмещениеФайла.Адрес = ПоместитьВоВременноеХранилище(Новый ДвоичныеДанные(Файл.ПолноеИмя), УникальныйИдентификатор);
		Иначе 
