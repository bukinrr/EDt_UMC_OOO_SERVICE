...
Процедура Инициализация()
	...
	ТабличныйДокумент.Область().Защита   = МедицинскаяОтчетность.ЗапрещатьРедактированиеФормМедОтчетности();
	...
	СохрКоличествоКопий = ХранилищеНастроекДанныхФорм.Загрузить(
		"Обработка.ОбщиеОбъектыМедОтчетности.Форма.ПечатьМедицинскихОтчетов",
		"ПредпросмотрМедицинскойОтчетностиКоличествоКопий");
	СохрРазобратьПоКопиям = ХранилищеНастроекДанныхФорм.Загрузить(
		"Обработка.ОбщиеОбъектыМедОтчетности.Форма.ПечатьМедицинскихОтчетов",
		"ПредпросмотрМедицинскойОтчетностиРазобратьПоКопиям");
	...
	Если СписокПечатаемыхЛистов.Количество() > 0
		И ВРег(СокрЛП(СписокПечатаемыхЛистов[0].Представление)) = ВРег(СокрЛП("СсылкаНаМедицинскийОтчет")) Тогда
		
		//УникальныеИдентификаторы = СписокПечатаемыхЛистов.ВыгрузитьЗначения();
		//УникальныеИдентификаторы.Удалить(0);
		//
		//СсылкаНаМедицинскийОтчет = СписокПечатаемыхЛистов.Получить(0);
		//
		//СписокПечатаемыхЛистов.Очистить();
		//СписокПечатаемыхЛистов.Добавить(СсылкаНаМедицинскийОтчет.Значение,
		//								СсылкаНаМедицинскийОтчет.Представление);
		//		
		//Запрос = Новый Запрос;
		//
		...
		//								
		//Запрос.УстановитьПараметр("МедицинскийОтчет", СписокПечатаемыхЛистов[0].Значение);
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ПроцессыОбработкиДокументов") Тогда
		...
		МодульПроцессыОбработкиДокументов.ПриИнициализацииФормыПечатиМедицинскихОтчетов(ЭтотОбъект, ДеревоЛистов);
...
&НаКлиенте
Функция ПолучитьПутьВыгрузки(ПутьДляВыгрузки = Неопределено)
...
Процедура СохранитьЗначениеНаСервере()
	...
	ХранилищеНастроекДанныхФорм.Сохранить("Обработка.ОбщиеОбъектыМедОтчетности.Форма.ПечатьМедицинскихОтчетов", "ПредпросмотрМедицинскойОтчетностиКоличествоКопий",     КоличествоКопий);
	ХранилищеНастроекДанныхФорм.Сохранить("Обработка.ОбщиеОбъектыМедОтчетности.Форма.ПечатьМедицинскихОтчетов", "ПредпросмотрМедицинскойОтчетностиРазобратьПоКопиям",   РазобратьПоКопиям);
		...
		Для Каждого Лист Из СписокЛистов.ПолучитьЭлементы() Цикл
			...
			Если Лист.Пометка Тогда
				...
				Если СписокПечатаемыхЛистов.Количество() > 0
					...
					= ВРег(СокрЛП("СсылкаНаМедицинскийОтчет"))
