...
Процедура ВосстановитьНастройкиОбновленияКонфигурации()
	...
	ПараметрыРаботыКлиента = СтандартныеПодсистемыКлиентПовтИсп.ПараметрыРаботыКлиента();
	Если ОбщегоНазначения.ИнформационнаяБазаФайловая() И Объект.РежимОбновления > 1 Тогда
	...
	#Если Не МобильныйКлиент Тогда
		...
		Для Каждого Привод Из Приводы Цикл
			Если Привод.DriveType <> 4 Тогда
				Продолжить;
			КонецЕсли;
			// 4 = CD-ROM drive, http://www.microsoft.com/technet/scriptcenter/guide/sas_scr_czxq.mspx?mfr=true.
			Если Привод.IsReady Тогда
