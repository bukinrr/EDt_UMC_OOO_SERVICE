...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ПустаяСтрока(ЗначенияПолей) Тогда
		...
		Если ЗначениеЗаполнено(Параметры.Объект) Тогда
			БИТ_КраткоеПредставление = ХранилищеПользовательскихНастроекОтчетов.Загрузить("ФормаЗаписиАдресаКраткоеПредставлениеАдреса", "КраткоеПредставление" + Параметры.Объект.Метаданные().Имя);
		Иначе
			БИТ_КраткоеПредставление = Истина;	
		КонецЕсли;
		//-бит
	ИначеЕсли УправлениеКонтактнойИнформациейКлиентСервер.ЭтоКонтактнаяИнформацияВJSON(ЗначенияПолей) Тогда
		...
		//+бит
		Если ДанныеАдреса.Свойство("КраткоеПредставление") Тогда
			БИТ_КраткоеПредставление = ДанныеАдреса.КраткоеПредставление;
		КонецЕсли;
		//-бит
	Иначе
		...
		НаселенныйПунктДетально = РаботаСАдресами.ПодготовитьАдресДляВвода(ДанныеАдреса);
		//+бит
		Если ТипЗнч(XDTOКонтактная.Состав.Состав) = Тип("ОбъектXDTO") Тогда
			БИТ_КраткоеПредставление = ОбщегоНазначенияКлиентСервер.СвойствоОбъекта(XDTOКонтактная.Состав.Состав, "КраткоеПредставление", Ложь);
		КонецЕсли;
		//-бит
	КонецЕсли;
	...
	//+бит
	Если ЗначениеЗаполнено(НаселенныйПункт) И Не ЗначениеЗаполнено(Улица) Тогда
		ЭтаФорма.ТекущийЭлемент = Элементы.Улица;
	КонецЕсли;
	//-бит
...
Процедура ПриОткрытии(Отказ)
	...
	Если Не ЗначениеЗаполнено(Улица) И БИТ_КраткоеПредставление Тогда
...
#Если ВебКлиент Или МобильныйКлиент Тогда
	// Удалить, если в будущих версиях платформы не будет требоваться.
...
Процедура ПодтвердитьИЗакрыть(Результат = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если Модифицированность Тогда // При немодифицированности работает как "отмена".
		...
		//+бит
		Контекст.Вставить("БИТ_КраткоеПредставление", БИТ_КраткоеПредставление);
		//-бит
...
#КонецЕсли
	...
	// +бит
	//Если редактировали адрес филиала, то обновляем кэш адресов.
	Если ТипЗнч(Параметры.Объект) = Тип("СправочникСсылка.Филиалы") Тогда	
		глКэшАдресовФилиалов = ОбщегоНазначенияСервер.ПолучитьАдресаФилиалов();		
	КонецЕсли;
	
	 СохранитьНастройкиФормы(БИТ_КраткоеПредставление, Параметры.Объект);
	// -бит
...
Функция РезультатВыбора(Контекст, ВозвращатьСписокЗначений = Ложь)
	...
	Если ТипЗнч(ДанныеВыбора) = Тип("Структура") Тогда
		...
		//+бит
		ДанныеВыбора.Вставить("КраткоеПредставление", Контекст.БИТ_КраткоеПредставление);
		//-бит
...
Процедура ОчиститьАдресКлиент()
	...
	//+БИТ
	НаселенныйПунктДетально.Вставить("КраткоеПредставление", БИТ_КраткоеПредставление);
	//-БИТ
...
Процедура ЗаполнитьПредопределенныеВариантыАдреса(Параметры)
	...
	Если ПустаяСтрока(Помещение) И ПустаяСтрока(ТипПомещения) Тогда
		//+бит Установка типа помещения по-умолчанию по типу владельца
		ТипВладельцаКИ = ТипЗнч(Параметры.Объект);
		Если ТипВладельцаКИ = Тип("СправочникСсылка.Контрагенты") Или ТипВладельцаКИ = Тип("СправочникСсылка.Филиалы") Тогда
			ТипПомещения = "Офис";
		Иначе
...
Процедура ОбновитьПредставлениеАдреса()
	...
	НаселенныйПунктДетально.Вставить("КраткоеПредставление", БИТ_КраткоеПредставление);
	//-БИТ
	РаботаСАдресамиКлиентСервер.ОбновитьПредставлениеАдреса(НаселенныйПунктДетально, ВидКонтактнойИнформации.МеждународныйФорматАдреса);
	ПредставлениеАдреса = НаселенныйПунктДетально.Value;
...
#Область БИТ_ДополнительныеПроцедуры //+бит

&НаКлиенте
Процедура БИТ_КраткоеПредставлениеПриИзменении(Элемент)
...
&НаСервереБезКонтекста
Процедура СохранитьНастройкиФормы(КраткоеПредставление, ПолучательКИ)

	Если ПолучательКИ <> Неопределено Тогда
		ИмяТипаПолучателяКИ = ПолучательКИ.Метаданные().Имя;
		ХранилищеПользовательскихНастроекОтчетов.Сохранить("ФормаЗаписиАдресаКраткоеПредставлениеАдреса", "КраткоеПредставление"+ИмяТипаПолучателяКИ,
