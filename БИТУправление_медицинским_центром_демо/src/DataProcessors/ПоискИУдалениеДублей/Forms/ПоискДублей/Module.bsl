...
Процедура ОбновитьОписаниеСостоянияНайденныхДублей(Форма)
	...
	Если ПустаяСтрока(Форма.ОписаниеОшибкиПоискаДублей) Тогда
		Описание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Найдено дублей: %2 (среди элементов: %1)'"),
			Форма.ВсегоЭлементов, Форма.ВсегоНайденоДублей);
	Иначе	
	...
	Форма.ОписаниеСостоянияНайденныхДублей = Новый ФорматированнаяСтрока(Описание + Символы.ПС
		+ НСтр("ru = 'Все отмеченные элементы будут помечены на удаление и заменены во всех местах использования на оригиналы (отмечены стрелкой).'"),
...
Функция ПарыЗаменДублей()
	...
	Для Каждого Родитель Из ДеревоДублей.Строки Цикл
		Если ЗначениеЗаполнено(Родитель.Пометка) Тогда // +бит
...
Функция ЗаполнитьРезультатыПоискаДублей(Знач АдресРезультата)
	...
	Если Не ПустаяСтрока(Данные.ОписаниеОшибки)
		И Найти(Данные.ОписаниеОшибки, "Показаны только первые") = 0
	Тогда // Фоновое сломалось
...
&НаКлиенте
Процедура бит_ВыбратьВсеГруппыЗамены(Команда)
	
	Для Каждого СтрокаГруппа Из НайденныеДубли.ПолучитьЭлементы() Цикл
		СтрокаГруппа.Пометка = 1;
	КонецЦикла;
	НайденныеДублиПометкаПриИзменении(Элементы.НайденныеДубли);
	
КонецПроцедуры

&НаКлиенте
Процедура бит_СнятьВсеГруппыЗамены(Команда)
	
	Для Каждого СтрокаГруппа Из НайденныеДубли.ПолучитьЭлементы() Цикл
		СтрокаГруппа.Пометка = 0;
	КонецЦикла;
	НайденныеДублиПометкаПриИзменении(Элементы.НайденныеДубли);
