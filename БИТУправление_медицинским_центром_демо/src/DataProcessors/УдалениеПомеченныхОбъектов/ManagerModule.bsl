...
//  ТипыУдаленныхОбъектовМассив	 - Массив	 - типы.
// 
// Возвращаемое значение:
//  ТаблицаЗначений - характеризующая ссылки на удаляемые объекты.
//
Функция УдалитьПомеченныеОбъекты(знач Удаляемые, ТипыУдаленныхОбъектовМассив) Экспорт
	...
	Пока УдаляемыеОбъекты.Количество() > 0 Цикл
		...
		Для Каждого СтрокаТаблицы Из НайденныеДанные Цикл
			Если ИсключенияПоискаСсылок.Получить(СтрокаТаблицы.Метаданные.ПолноеИмя()) = Неопределено Тогда
