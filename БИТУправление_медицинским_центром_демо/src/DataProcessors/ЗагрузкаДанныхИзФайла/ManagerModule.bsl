	...
	Для НомерСтроки = ПерваяСтрокаТаблицы По ШаблонСДанными.ВысотаТаблицы Цикл 
		...
		Для НомерКолонки = 1 По ШаблонСДанными.ШиринаТаблицы Цикл
			...
			Если Колонка <> Неопределено Тогда
				...
				//+БИТ 
				//Конвертируем строку в дату, чтобы не нужно было писать дату со временем
				ИначеЕсли ТипДанных = Тип("Дата") Тогда
					КвалификаторДаты = СтрРазделить(Колонка.Примечание,Символы.ПС)[1];
					Если КвалификаторДаты = "Дата" Тогда
						ДанныеЯчейки = СтроковыеФункцииКлиентСервер.СтрокаВДату(Ячейка.Текст);
					Иначе 
...
Процедура ИнформациюПоКолонкамИзРеквизитовСправочника(ПараметрыЗагрузки, ИнформацияПоКолонкам)
	...
	Для каждого Реквизит Из МетаданныеСправочника.Реквизиты Цикл
		...
		ИначеЕсли Реквизит.Тип.СодержитТип(Тип("Дата")) Тогда
			...
			//+БИТ
			//Добавляем в подсказку формат даты
			ОписаниеТипаКолонки = ОписаниеТипаКолонки + Символы.ПС + "Если предполагается ввод даты без времени, то формат даты должен быть в виде ""ДД.ММ.ГГГГ"" или ""ДД/ММ/ГГ""" 
								 + Символы.ПС + "Например, ""23.02.1980"" или ""23/02/80""";	
			//-БИТ
...
Процедура ЗаписатьСопоставленныеДанные(ПараметрыВыгрузки, АдресХранилища) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.КонтактнаяИнформация") Тогда
		ВидыКонтактнойИнформации = КонтактнаяИнформацияСерверПереопределяемый.ПолучитьВидыКонтактнойИнформацииМетаданных(ИмяСправочника);
	КонецЕсли;
			...
			Для каждого Колонка Из ИнформацияПоКолонкам Цикл 
				Если Колонка.Видимость Тогда
					Если СтрНачинаетсяС(Колонка.ИмяКолонки, "КонтактнаяИнформация_") Тогда
						...
						ВидКонтактнойИнформации = ВидыКонтактнойИнформации.Найти(ИмяВидаКИ);
