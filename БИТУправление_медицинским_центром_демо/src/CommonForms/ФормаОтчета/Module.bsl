...
Процедура УстановитьВидимостьДоступность(ОбновитьКоманды = Истина)
	...
	КонецЕсли;
	//+бит
	Заголовок = Заголовок + " (" + НастройкиОтчета.Наименование + ")";
	//-бит
	Если РежимРасшифровки Тогда
...
Процедура ЗаполнитьКомандыВыбораВариантов()
	...
	ГруппаВариантыОтчета = Элементы.ВариантыОтчета;
	ГруппаПользовательскиеВарианты = Элементы.битПользовательскиеВариантыОтчетов;	
	//-бит
	...
	Для Каждого СтрокаТаблицы Из ТаблицаВариантов Цикл
		...
		//+бит
		Группа = ГруппаВариантыОтчета;
		Попытка
			УИД = Новый УникальныйИдентификатор(СтрокаТаблицы.КлючВарианта);
			// Это пользовательский вариант
			Группа = ГруппаПользовательскиеВарианты;
		Исключение
		КонецПопытки;
		//-бит
...
&НаКлиенте
Процедура бит_ОткрытьФормуСпискаВариантовОтчета(Команда)
	
	Отбор = Новый Структура("Отчет", НастройкиОтчета.ОтчетСсылка);
	ОткрытьФорму("Справочник.ВариантыОтчетов.ФормаСписка", Новый Структура("Отбор", Отбор),,НастройкиОтчета.ОтчетСсылка);
...
&НаКлиенте
Процедура Подключаемый_бит_ИзменитьУровеньГруппировки(Команда)
	
	ОтчетыКлиентПереопределяемый.ИзменениеУровняГруппировки(ЭтотОбъект, Команда.Имя);
