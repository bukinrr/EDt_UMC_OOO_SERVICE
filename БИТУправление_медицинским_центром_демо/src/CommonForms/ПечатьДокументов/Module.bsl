...
Процедура ОткрытьВExcel(Команда)
	...
	#Если ВебКлиент Или МобильныйКлиент Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("В режиме Веб-клиента данная операция не доступна.");
	#Иначе
	
	ИмяВременногоФайла=ПолучитьИмяВременногоФайла("xls");
	ТекущаяПечатнаяФорма.Записать(ИмяВременногоФайла,"xls");
	Попытка
		ЗапуститьПриложение(ИмяВременногоФайла);
	Исключение
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("ru = 'Необходимо установить MS Excel'");
	КонецПопытки;
...
Функция СформироватьПечатныеФормы(ИменаМакетов, Отказ)
	...
	Если ЗначениеЗаполнено(Параметры.ИсточникДанных) Тогда
		//+БИТ
		Результат = УправлениеПечатью.ПодготовитьКоллекциюПечатныхФорм(ИменаМакетов);
		Если ТипЗнч(Параметры.ИсточникДанных) = Тип("Массив") Тогда 
			СоставПакета = ПечатьДокументовСервер.ПередатьСоставВКлиент(Параметры.ИсточникДанных, Параметры.ПараметрКоманды);
		//	СтруктураНаПечать = ПечатьДокументовКлиент.НапечататьПакетДокументов(СоставПакета, Параметры.ПараметрКоманды, Параметры.ИсточникДанных, Ложь);
		//	Результат = ПечатьДокументовКлиент.ВывестиНапечататьДокумент(СтруктураНаПечать,ДокСсылка);
		Иначе
			СтрокаКнопки = Новый Структура("СсылкаНаВнешнююОбработку, ВидПечатнойФормы,	ЗаменяемаяПечатнаяФорма, НомерСтроки", Параметры.ИсточникДанных, , , 1);
			СтруктураПечати = ПечатьДокументовСервер.ВывестиНапечататьДокумент(Параметры.ИсточникДанных, "Структура", Параметры.ПараметрКоманды, Параметры.ИмяМенеджераПечати, СтрокаКнопки);
			Результат[0].ТабличныйДокумент = СтруктураПечати.ПечДокумент;
		КонецЕсли;
		//-БИТ
	Иначе
...
Процедура СоздатьРеквизитыИЭлементыФормыДляПечатныхФорм(КоллекцияПечатныхФорм)
	...
	Для Каждого РеквизитФормы Из НовыеРеквизитыФормы Цикл
		...
		ОписаниеПечатнойФормы.Экземпляров = ?(ОписаниеПечатнойФормы.Экземпляров = Неопределено, 1, ОписаниеПечатнойФормы.Экземпляров); //+БИТ
...
Процедура НастроитьВидимостьЭлементовФормы(Знач ЕстьРазрешенныйВывод)
	...
	Элементы.КнопкаСохранить.Видимость = ЕстьДанныеДляПечати И ЕстьРазрешенныйВывод; //+БИТ И ЕстьРазрешенноеРедактирование;
	...
	ДоступноИзменениеМакетов = ЕстьРедактируемыеМакеты();
...
Процедура УстановитьТекущуюСтраницу()
	...
	//+БИТ
	Если Не Элементы.ТекущаяПечатнаяФорма.Редактирование Тогда
		ПереключитьРедактированиеТекущейПечатнойФормы(); // Выключим редактирование по умолчанию.
	КонецЕсли;
	//-БИТ
