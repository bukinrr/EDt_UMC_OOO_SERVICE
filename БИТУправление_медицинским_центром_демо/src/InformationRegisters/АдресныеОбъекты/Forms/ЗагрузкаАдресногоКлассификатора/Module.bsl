...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если РежимРаботы = "ЗАГРУЗКА" Тогда
		...
		Элементы.бит_ОтключитьИнтернетПоддержку.Видимость           = Элементы.ИнформацияОбОбновление.Видимость;
...
&НаКлиенте
Процедура СсылкаНаФайлыФИАСНажатие(Элемент)
	ЗапуститьПриложение("http://its.1c.ru/download/fias2");
КонецПроцедуры
...
Процедура бит_ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	Если ИмяСобытия = "ОчищенАдресныйКлассификатор" Тогда
		Для Каждого ОчищенныйСубъектРФ Из Источник.СубъектыРФ.НайтиСтроки(Новый Структура("Очищать", Истина)) Цикл
			Для Каждого СубъектРФ Из СубъектыРФ.НайтиСтроки(Новый Структура("КодСубъектаРФ", ОчищенныйСубъектРФ.КодСубъектаРФ)) Цикл
				СубъектРФ.Загружено = Ложь;
				СубъектРФ.Загружать = Ложь;
			КонецЦикла;
		КонецЦикла;
	КонецЕсли;
КонецПроцедуры
