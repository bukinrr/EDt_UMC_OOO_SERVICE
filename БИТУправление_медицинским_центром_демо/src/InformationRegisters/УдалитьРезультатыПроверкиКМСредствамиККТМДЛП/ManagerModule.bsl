...
Процедура ЗаписатьРезультатыПроверкиКМ(Параметры) Экспорт
	...
	Попытка
		...
		ЭлементБлокировкиДанных = БлокировкаДанных.Добавить(Метаданные.РегистрыСведений.УдалитьРезультатыПроверкиКМСредствамиККТМДЛП.ПолноеИмя());
	...
	Исключение
		...
		ОтменитьТранзакцию();
		ИмяСобытия = НСтр("ru = 'Проверка КМ средствами ККТ.Запись результатов'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция ПолучитьРезультатыПроверкиКМ(Параметры) Экспорт
	...
	Если ЗначениеЗаполнено(НомераУпаковок) Тогда
		...
		|ИЗ
		|	РегистрСведений.УдалитьРезультатыПроверкиКМСредствамиККТМДЛП КАК РезультатыПроверки
	...
	Для Каждого Строка Из Таблица Цикл
		...
		РезультатПроверкиКМ.Вставить("КлючГруппыНастроекКонтроляКМ", ГруппыНастроекКонтроляКМ.ПараметрыКонтроляСредствамиККТ);
...
Функция СвойстваРезультатаПроверкиКМ() Экспорт
	...
	Реквизиты = Метаданные.РегистрыСведений.УдалитьРезультатыПроверкиКМСредствамиККТМДЛП.Реквизиты;
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|	МАКСИМУМ(РезультатыПроверкиКМСредствамиККТ.Документ)         КАК Документ,
	...
	|	РезультатыПроверкиКМСредствамиККТ.НомерУпаковки  КАК НомерУпаковки,
	|	РезультатыПроверкиКМСредствамиККТ.КлючУпаковки   КАК КлючУпаковки,
	|	РезультатыПроверкиКМСредствамиККТ.Документ       КАК Документ
	|ИЗ
	...
	|		ПО
	|			РезультатыПроверкиКМСредствамиРВ.НомерУпаковки = РезультатыПроверкиКМСредствамиККТ.НомерУпаковки
	|			И РезультатыПроверкиКМСредствамиРВ.КлючУпаковки = РезультатыПроверкиКМСредствамиККТ.КлючУпаковки
	|			И (НЕ РезультатыПроверкиКМСредствамиККТ.КонтрольПройден ИЛИ РезультатыПроверкиКМСредствамиРВ.КонтрольПройден)
	|ГДЕ
	|	РезультатыПроверкиКМСредствамиРВ.НомерУпаковки ЕСТЬ NULL
	|");
	
	ДополнительныеПараметры = ОбновлениеИнформационнойБазы.ДополнительныеПараметрыОтметкиОбработки();
	ДополнительныеПараметры.ЭтоНезависимыйРегистрСведений = Истина;
	ДополнительныеПараметры.ПолноеИмяРегистра = Метаданные.РегистрыСведений.УдалитьРезультатыПроверкиКМСредствамиККТМДЛП.ПолноеИмя();
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ПолноеИмяОбъекта = Метаданные.РегистрыСведений.УдалитьРезультатыПроверкиКМСредствамиККТМДЛП.ПолноеИмя();
	...
	//	|		ЛЕВОЕ СОЕДИНЕНИЕ
	//	|			РегистрСведений.УдалитьРезультатыПроверкиКМСредствамиККТМДЛП КАК ДанныеРегистра
	//	|		ПО
	...
	//			КонецЕсли;
	//			
	//			НаборОтметить = РегистрыСведений.УдалитьРезультатыПроверкиКМСредствамиККТМДЛП.СоздатьНаборЗаписей();
