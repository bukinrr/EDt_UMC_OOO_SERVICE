...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Для Каждого КлючИЗначение Из Параметры.РезультатыПроверки Цикл
		...
		Представление = КонтрольКодовМаркировкиМДЛПКлиентСервер.ПредставлениеРезультатаПроверкиКМСредствамиККТ(РезультатЭлементаПроверки);
...
Процедура КомандаПодтвердитьРезультатыПроверки(Команда)
	...
	СвойстваРезультата = СтрСоединить(СвойстваРезультатаПроверки(), ",");
	
	Результаты = Новый Соответствие;
	Для Каждого Запись Из РезультатыПроверки Цикл
		
		Результат = Новый Структура(СвойстваРезультата);
...
&НаСервере
Функция СвойстваРезультатаПроверки()
	
	Возврат РегистрыСведений.УдалитьРезультатыПроверкиКМСредствамиККТМДЛП.СвойстваРезультатаПроверкиКМ();
