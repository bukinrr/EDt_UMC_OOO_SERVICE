...
// Продолжение записи после ответа на возможные предупреждения при записи.
//
// Параметры:
//  РезультатВопроса		 - КодВозвратаДиалога	 - ответ пользователя.
//  ДополнительныеПараметры	 - Структура			 - параметры контектста выполнения.
//
&НаКлиенте
...
Функция НастройкаОбработкиПисемВДругомКлиентеДоступна()
	...
	ИспользоватьПризнакРассмотрено = Ложь;
