...
Процедура ВидимостьДоступностьКорректность(Форма, Изменения = "")
	...
	Если Изменения = "" Или Изменения = "ВидРассылки" Тогда
		...
		Элементы.ВидыРассылкиЛичная.Доступность = Объект.Личная;
...
Функция ПочтовыеАдресаПолучателя(Получатель, СписокЗначений)
	...
	ПочтовыйАдрес = КонтактнаяИнформацияСервер.ПолучитьКИОбъекта(Получатель,ТипКИ,,Истина);
	Если ЗначениеЗаполнено(ПочтовыйАдрес) Тогда
		СписокЗначений.Добавить(ВидКИ, ПочтовыйАдрес);
	КонецЕсли;
...
Процедура ПодключитьКэшНастроекЭлектроннойПочты()
	...
	Для Каждого СтрокаПолучатель Из ТаблицаТиповПолучателей Цикл
		...
		Если СтрокаПолучатель.ИОМД 	= Объект.ТипПолучателейРассылки Или Объект.ТипПолучателейРассылки = Справочники.ИдентификаторыОбъектовМетаданных.ПустаяСсылка() Тогда
			Объект.ТипПолучателейРассылки 	= СтрокаПолучатель.ИОМД;
...
Функция ИнициализироватьОтчет(СтрокаОтчеты, ДобавитьДежурнуюФразу, ПользовательскиеНастройки, Интерактивно = Истина)
	...
	Если ПараметрыОтчета.Инициализирован Тогда
		...
		Если ЗначениеЗаполнено(ПараметрыОтчета.СсылкаВарианта) Тогда
			...
			//+бит
			Если ТипЗнч(СтрокаОтчеты.Отчет) = Тип("СправочникСсылка.ВариантыОтчетов") Тогда
				СтрокаОтчеты.Представление = СтрокаОтчеты.Представление + " (" + Строка(СтрокаОтчеты.Отчет.Отчет) + ")";
			КонецЕсли;
			//-бит
		КонецЕсли;
	КонецЕсли;
...
////	Если Объект.ИспользоватьЭлектроннуюПочту Тогда
////		РаботаСПочтовымиСообщениямиКлиент.ПроверитьУчетнуюЗапись(Объект.УчетнаяЗапись);
////	КонецЕсли;	
////КонецПроцедуры

&НаКлиенте
Процедура ОткрытьПомощникНастройки(Команда)
	ОткрытьФорму("Справочник.УчетныеЗаписиЭлектроннойПочты.Форма.ПомощникНастройкиУчетнойЗаписи",Новый Структура("Ключ", Объект.УчетнаяЗапись));
КонецПроцедуры
//-БИТ	
