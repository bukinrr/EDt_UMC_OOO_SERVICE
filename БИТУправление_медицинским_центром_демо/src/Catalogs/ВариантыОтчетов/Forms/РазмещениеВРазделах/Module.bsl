...
Процедура ПриОткрытии(Отказ)
	Если СообщенияОбОшибках <> Неопределено Тогда
		Отказ = Истина;
		ОчиститьСообщения();
		СтандартныеПодсистемыКлиент.ВывестиПредупреждение(ЭтотОбъект, СообщенияОбОшибках);
	КонецЕсли;
КонецПроцедуры
...
Процедура ЗаполнитьРазделы()
	...
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ПолныеПраваНаВарианты",        РольДоступна("ПолныеПрава"));
