...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//+бит
	СозданиеКнопкиСохраненияПароля();
	//-бит
...
//+бит
&НаКлиенте
Процедура Бит_СохранитьПарольЭЦП(Команда)
	
	Если Не ОбщегоНазначения.ЕстьПравоДоступа("Изменение", "Справочник.СертификатыКлючейЭлектроннойПодписиИШифрования", ОбщегоНазначения.ТекущийПользователь()) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Недостаточно прав для установки и изменения пароля ЭЦП.'"));
		Возврат;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
		ОткрытьФорму("Справочник.СертификатыКлючейЭлектроннойПодписиИШифрования.Форма.бит_СохранениеПароляЭЦП",
				Новый Структура("СертификатЭЦП", Объект.Ссылка), ЭтаФорма,,,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	Иначе
...
//+бит
&НаСервере
Процедура СозданиеКнопкиСохраненияПароля()
	
	Команда = Команды.Добавить("Бит_СохранитьПарольЭЦП");
	Команда.Заголовок = "Сохранить пароль";
	Команда.Действие  = "Бит_СохранитьПарольЭЦП";
	
	КнопкаФормы = Элементы.Добавить("Бит_СохранитьПарольЭЦП", Тип("КнопкаФормы"), Элементы.ФормаСтандартныеКоманды);
	КнопкаФормы.ИмяКоманды = "Бит_СохранитьПарольЭЦП";
