...
Процедура ГенерироватьНовыйКод() Экспорт
	...
	Если ЭтоГруппа ИЛИ Уровень() = 0 Тогда
		...
		Пока НЕ Справочники.МедицинскиеОтчеты.НайтиПоКоду(Код) =
				Справочники.МедицинскиеОтчеты.ПустаяСсылка() Цикл
...
Процедура ПередЗаписью(Отказ)
	...
	Если Не ЭтоНовый() Тогда
		...
		Если ПрежнийРодитель <> Родитель Тогда
			...
			РеглОтчеты = Справочники.МедицинскиеОтчеты;
	...
	Если Не ЭтоГруппа И Не ЗначениеЗаполнено(УИДОтчета) Тогда 
		УИДОтчета = Новый УникальныйИдентификатор;	
	КонецЕсли;
	
	Если Не ЭтоГруппа И Не ЭтоНовый() Тогда
		
		ИсточникОтчетаИсх = Ссылка.ИсточникОтчета;
		Если ИсточникОтчета <> ИсточникОтчетаИсх
			Или ПометкаУдаления И Не Ссылка.ПометкаУдаления
		Тогда
			ЭтоВстроенныйОтчет = Ложь;
			ВстроенныеОтчеты = Справочники.МедицинскиеОтчеты.ПереченьОтчетов();
			Для Каждого ОписаниеОтчета Из ВстроенныеОтчеты Цикл
				Если ИсточникОтчетаИсх = ОписаниеОтчета.ИмяМетаданных Тогда
			...
			Если ЭтоВстроенныйОтчет Тогда
				Отказ = Истина;
				Если ИсточникОтчета <> ИсточникОтчетаИсх Тогда
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Изменение элементов встроенной отчетности не допускается.'"));
				Иначе
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Пометка на удаление элементов встроенной отчетности не допускается.'"));
				КонецЕсли;
				Возврат;
			КонецЕсли;
		КонецЕсли;
